...
const CLAIRDASH_APP_NAME = 'clairdash_webapp';
...
const TrackingProviderMain: FC<React.PropsWithChildren<TrackingData>> = ({
    ...
    const getClairdashPageProperties = useCallback(
    ...
    const clairdashContext = useMemo<rudderSDK.apiOptions>(
        () => ({
            app: appContext as any as rudderSDK.apiObject,
            page: getClairdashPageProperties(
                pageContext,
            ) as any as rudderSDK.apiObject,
        }),
        [appContext, pageContext, getClairdashPageProperties],
    );
    ...
    const page = useCallback(
        (rudderPageEvent: PageData): void => {
            const newPageContext = getClairdashPageProperties(rudderPageEvent);
        ...
        [rudderAnalytics, clairdashContext, getClairdashPageProperties],
    );

    const track = useCallback(
        ({ name, properties = {} }: EventData): void => {
            rudderAnalytics?.track(
                `${CLAIRDASH_APP_NAME}.${name}`,
                properties,
                {
                    ...clairdashContext,
                    section: sectionContext,
        ...
        [rudderAnalytics, sectionContext, clairdashContext],
    );
    const identify = useCallback(
        ({ id, traits }: IdentifyData) => {
            if (mode && mode !== ClairdashMode.DEMO) {
                rudderAnalytics?.identify(id, traits, clairdashContext);
            }
        },
        [clairdashContext, mode, rudderAnalytics],
    );

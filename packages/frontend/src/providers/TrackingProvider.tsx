...
const BIGBYTES_APP_NAME = 'bigbytes_webapp';
...
const TrackingProviderMain: FC<React.PropsWithChildren<TrackingData>> = ({
    ...
    const getBigbytesPageProperties = useCallback(
    ...
    const bigbytesContext = useMemo<rudderSDK.apiOptions>(
        () => ({
            app: appContext as any as rudderSDK.apiObject,
            page: getBigbytesPageProperties(
                pageContext,
            ) as any as rudderSDK.apiObject,
        }),
        [appContext, pageContext, getBigbytesPageProperties],
    );
    ...
    const page = useCallback(
        (rudderPageEvent: PageData): void => {
            const newPageContext = getBigbytesPageProperties(rudderPageEvent);
        ...
        [rudderAnalytics, bigbytesContext, getBigbytesPageProperties],
    );

    const track = useCallback(
        ({ name, properties = {} }: EventData): void => {
            rudderAnalytics?.track(
                `${BIGBYTES_APP_NAME}.${name}`,
                properties,
                {
                    ...bigbytesContext,
                    section: sectionContext,
        ...
        [rudderAnalytics, sectionContext, bigbytesContext],
    );
    const identify = useCallback(
        ({ id, traits }: IdentifyData) => {
            if (mode && mode !== BigbytesMode.DEMO) {
                rudderAnalytics?.identify(id, traits, bigbytesContext);
            }
        },
        [bigbytesContext, mode, rudderAnalytics],
    );

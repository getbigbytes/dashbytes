...
const getScheduler = async (uuid: string) =>
    bigbytesApi<SchedulerAndTargets>({
        url: `/schedulers/${uuid}`,
        method: 'GET',
        body: undefined,
    });
...
>(
    ...
    bigbytesApi<T extends ApiJobStatusResponse['results'] ? T : never>({
...
const sendNowScheduler = async (scheduler: CreateSchedulerAndTargets) =>
    bigbytesApi<ApiTestSchedulerResponse['results']>({
        url: `/schedulers/send`,
        method: 'POST',

import { ClairdashMode, SessionUser } from '@clairdash/common';
...
const printMessage = (
    ...
    return `${info.timestamp} [Clairdash]${jobId}${serviceName} ${info.level}: ${info.message}`;
};
...
const transports = [];
if (clairdashConfig.logging.outputs.includes('console')) {
    transports.push(
        new winston.transports.Console({
            format: formatters[
                clairdashConfig.logging.consoleFormat ||
                    clairdashConfig.logging.format
            ],
            level:
                clairdashConfig.logging.consoleLevel ||
                clairdashConfig.logging.level,
        }),
    );
}
if (clairdashConfig.logging.outputs.includes('file')) {
    transports.push(
        new winston.transports.File({
            filename: clairdashConfig.logging.filePath,
            format: formatters[
                clairdashConfig.logging.fileFormat ||
                    clairdashConfig.logging.format
            ],
            level:
                clairdashConfig.logging.fileLevel ||
                clairdashConfig.logging.level,
        }),
    );
}
...
export const expressWinstonPreResponseMiddleware: express.RequestHandler = (
    ...
    if (clairdashConfig.mode !== ClairdashMode.DEV) {
        winstonLogger.log({
            level: 'http',

import { BigbytesMode, SessionUser } from '@bigbytes/common';
...
const printMessage = (
    ...
    return `${info.timestamp} [Bigbytes]${jobId}${serviceName} ${info.level}: ${info.message}`;
};
...
const transports = [];
if (bigbytesConfig.logging.outputs.includes('console')) {
    transports.push(
        new winston.transports.Console({
            format: formatters[
                bigbytesConfig.logging.consoleFormat ||
                    bigbytesConfig.logging.format
            ],
            level:
                bigbytesConfig.logging.consoleLevel ||
                bigbytesConfig.logging.level,
        }),
    );
}
if (bigbytesConfig.logging.outputs.includes('file')) {
    transports.push(
        new winston.transports.File({
            filename: bigbytesConfig.logging.filePath,
            format: formatters[
                bigbytesConfig.logging.fileFormat ||
                    bigbytesConfig.logging.format
            ],
            level:
                bigbytesConfig.logging.fileLevel ||
                bigbytesConfig.logging.level,
        }),
    );
}
...
export const expressWinstonPreResponseMiddleware: express.RequestHandler = (
    ...
    if (bigbytesConfig.mode !== BigbytesMode.DEV) {
        winstonLogger.log({
            level: 'http',

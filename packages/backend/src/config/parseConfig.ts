...
const parseLoggingLevel = (raw: string): LoggingLevel => {
    if (!assertIsLoggingLevel(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "BIGBYTES_LOG_LEVEL". Value must be one of "error", "warn", "info", "debug" but BIGBYTES_LOG_LEVEL=${raw}`,
        );
    }
    return raw;
};
...
const parseLoggingFormat = (raw: string): LoggingFormat => {
    if (!assertIsLoggingFormat(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "BIGBYTES_LOG_FORMAT". Value must be one of "json", "plain", "pretty" but BIGBYTES_LOG_FORMAT=${raw}`,
        );
    }
    return raw;
};
...
const parseLoggingOutput = (raw: string): LoggingOutput => {
    if (!assertIsLoggingOutput(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "BIGBYTES_LOG_OUTPUT". Value must be one of "console", "file" but BIGBYTES_LOG_OUTPUT=${raw}`,
        );
    }
    return raw;
};
...
export type BigbytesConfig = {
    bigbytesSecret: string;
    ...
    siteUrl: string;
    staticIp: string;
    bigbytesCloudInstance: string | undefined;
    k8s: {
...
export type AuthConfig = {
    ...
    /**
     * @deprecated Group Sync is deprecated. https://github.com/getbigbytes/bigbytes/issues/12430
     */
...
export const parseConfig = (): BigbytesConfig => {
    const bigbytesSecret = process.env.BIGBYTES_SECRET;
    if (!bigbytesSecret) {
        throw new ParseError(
            `Must specify environment variable BIGBYTES_SECRET. Keep this value hidden!`,
            {},
        );
    }
    const bigbytesMode = process.env.BIGBYTES_MODE;
    if (bigbytesMode !== undefined && !isBigbytesMode(bigbytesMode)) {
        throw new ParseError(
            `Bigbytes mode set by environment variable BIGBYTES_MODE=${bigbytesMode} is invalid. Must be one of ${Object.values(
                BigbytesMode,
            )}`,
            {},
        );
    ...
    const mode = bigbytesMode || BigbytesMode.DEFAULT;
    ...
    if (
        ...
        console.log(
            `WARNING: Using ${siteUrl} as the base SITE_URL for Bigbytes. This is not suitable for production. Update with a top-level domain using https such as https://bigbytes.mycompany.com`,
        );
    }
    ...
    return {
        ...
        security: {
            contentSecurityPolicy: {
                reportOnly: process.env.BIGBYTES_CSP_REPORT_ONLY !== 'false', // defaults to true
                allowedDomains: (
                    process.env.BIGBYTES_CSP_ALLOWED_DOMAINS || ''
                )
                    .split(',')
                    .map((domain) => domain.trim()),
                reportUri: process.env.BIGBYTES_CSP_REPORT_URI,
            },
        },
        smtp: process.env.EMAIL_SMTP_HOST
            ? {
                  ...
                  sender: {
                      name: process.env.EMAIL_SMTP_SENDER_NAME || 'Bigbytes',
        ...
        bigbytesCloudInstance: process.env.BIGBYTES_CLOUD_INSTANCE,
        k8s: {
        ...
        prometheus: {
            enabled: process.env.BIGBYTES_PROMETHEUS_ENABLED === 'true',
            port:
                getIntegerFromEnvironmentVariable(
                    'BIGBYTES_PROMETHEUS_PORT',
                ) ?? 9090,
            path: process.env.BIGBYTES_PROMETHEUS_PATH || '/metrics',
            prefix: process.env.BIGBYTES_PROMETHEUS_PREFIX,
            gcDurationBuckets: getFloatArrayFromEnvironmentVariable(
                'BIGBYTES_GC_DURATION_BUCKETS',
            ),
            eventLoopMonitoringPrecision: getIntegerFromEnvironmentVariable(
                'BIGBYTES_EVENT_LOOP_MONITORING_PRECISION',
            ),
            labels: getObjectFromEnvironmentVariable(
                'BIGBYTES_PROMETHEUS_LABELS',
            ),
        },
        allowMultiOrgs: process.env.ALLOW_MULTIPLE_ORGS === 'true',
        maxPayloadSize: process.env.BIGBYTES_MAX_PAYLOAD || '5mb',
        query: {
            maxLimit:
                getIntegerFromEnvironmentVariable(
                    'BIGBYTES_QUERY_MAX_LIMIT',
                ) || 5000,
            csvCellsLimit:
                getIntegerFromEnvironmentVariable(
                    'BIGBYTES_CSV_CELLS_LIMIT',
                ) || 100000,
            timezone: process.env.BIGBYTES_QUERY_TIMEZONE,
        },
        chart: {
            versionHistory: {
                daysLimit:
                    getIntegerFromEnvironmentVariable(
                        'BIGBYTES_CHART_VERSION_HISTORY_DAYS_LIMIT',
                    ) || 3,
            },
        },
        customVisualizations: {
        ...
        pivotTable: {
            maxColumnLimit:
                getIntegerFromEnvironmentVariable(
                    'BIGBYTES_PIVOT_TABLE_MAX_COLUMN_LIMIT',
                ) || 60,
        },
        s3: {
        ...
        headlessBrowser: {
            ...
            internalBigbytesHost:
                process.env.INTERNAL_BIGBYTES_HOST || siteUrl,
        },
        resultsCache: {
        ...
        logging: {
            level: parseLoggingLevel(
                process.env.BIGBYTES_LOG_LEVEL ||
                    ((process.env.NODE_ENV || 'development') === 'development'
                        ? 'debug'
                        : 'http'),
            ),
            format: parseLoggingFormat(
                process.env.BIGBYTES_LOG_FORMAT || 'pretty',
            ),
            outputs: (process.env.BIGBYTES_LOG_OUTPUTS
                ? process.env.BIGBYTES_LOG_OUTPUTS.split(',')
                : ['console']
            ).map(parseLoggingOutput),
            consoleFormat:
                process.env.BIGBYTES_LOG_CONSOLE_FORMAT === undefined
                    ? undefined
                    : parseLoggingFormat(
                          process.env.BIGBYTES_LOG_CONSOLE_FORMAT,
                      ),
            consoleLevel:
                process.env.BIGBYTES_LOG_CONSOLE_LEVEL === undefined
                    ? undefined
                    : parseLoggingLevel(
                          process.env.BIGBYTES_LOG_CONSOLE_LEVEL,
                      ),
            fileFormat:
                process.env.BIGBYTES_LOG_FILE_FORMAT === undefined
                    ? undefined
                    : parseLoggingFormat(process.env.BIGBYTES_LOG_FILE_FORMAT),
            fileLevel:
                process.env.BIGBYTES_LOG_FILE_LEVEL === undefined
                    ? undefined
                    : parseLoggingLevel(process.env.BIGBYTES_LOG_FILE_LEVEL),
            filePath: process.env.BIGBYTES_LOG_FILE_PATH || './logs/all.log',
        },
        github: {
            appName: process.env.GITHUB_APP_NAME || 'bigbytes-app-dev',
            redirectDomain:

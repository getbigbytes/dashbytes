...
const parseLoggingLevel = (raw: string): LoggingLevel => {
    if (!assertIsLoggingLevel(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "CLAIRDASH_LOG_LEVEL". Value must be one of "error", "warn", "info", "debug" but CLAIRDASH_LOG_LEVEL=${raw}`,
        );
    }
    return raw;
};
...
const parseLoggingFormat = (raw: string): LoggingFormat => {
    if (!assertIsLoggingFormat(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "CLAIRDASH_LOG_FORMAT". Value must be one of "json", "plain", "pretty" but CLAIRDASH_LOG_FORMAT=${raw}`,
        );
    }
    return raw;
};
...
const parseLoggingOutput = (raw: string): LoggingOutput => {
    if (!assertIsLoggingOutput(raw)) {
        throw new ParseError(
            `Cannot parse environment variable "CLAIRDASH_LOG_OUTPUT". Value must be one of "console", "file" but CLAIRDASH_LOG_OUTPUT=${raw}`,
        );
    }
    return raw;
};
...
export type ClairdashConfig = {
    clairdashSecret: string;
    ...
    siteUrl: string;
    staticIp: string;
    clairdashCloudInstance: string | undefined;
    k8s: {
...
export type AuthConfig = {
    ...
    /**
     * @deprecated Group Sync is deprecated. https://github.com/clairview/clairdash/issues/12430
     */
...
export const parseConfig = (): ClairdashConfig => {
    const clairdashSecret = process.env.CLAIRDASH_SECRET;
    if (!clairdashSecret) {
        throw new ParseError(
            `Must specify environment variable CLAIRDASH_SECRET. Keep this value hidden!`,
            {},
        );
    }
    const clairdashMode = process.env.CLAIRDASH_MODE;
    if (clairdashMode !== undefined && !isClairdashMode(clairdashMode)) {
        throw new ParseError(
            `Clairdash mode set by environment variable CLAIRDASH_MODE=${clairdashMode} is invalid. Must be one of ${Object.values(
                ClairdashMode,
            )}`,
            {},
        );
    ...
    const mode = clairdashMode || ClairdashMode.DEFAULT;
    ...
    if (
        ...
        console.log(
            `WARNING: Using ${siteUrl} as the base SITE_URL for Clairdash. This is not suitable for production. Update with a top-level domain using https such as https://clairdash.mycompany.com`,
        );
    }
    ...
    return {
        ...
        security: {
            contentSecurityPolicy: {
                reportOnly: process.env.CLAIRDASH_CSP_REPORT_ONLY !== 'false', // defaults to true
                allowedDomains: (
                    process.env.CLAIRDASH_CSP_ALLOWED_DOMAINS || ''
                )
                    .split(',')
                    .map((domain) => domain.trim()),
                reportUri: process.env.CLAIRDASH_CSP_REPORT_URI,
            },
        },
        smtp: process.env.EMAIL_SMTP_HOST
            ? {
                  ...
                  sender: {
                      name: process.env.EMAIL_SMTP_SENDER_NAME || 'Clairdash',
        ...
        clairdashCloudInstance: process.env.CLAIRDASH_CLOUD_INSTANCE,
        k8s: {
        ...
        prometheus: {
            enabled: process.env.CLAIRDASH_PROMETHEUS_ENABLED === 'true',
            port:
                getIntegerFromEnvironmentVariable(
                    'CLAIRDASH_PROMETHEUS_PORT',
                ) ?? 9090,
            path: process.env.CLAIRDASH_PROMETHEUS_PATH || '/metrics',
            prefix: process.env.CLAIRDASH_PROMETHEUS_PREFIX,
            gcDurationBuckets: getFloatArrayFromEnvironmentVariable(
                'CLAIRDASH_GC_DURATION_BUCKETS',
            ),
            eventLoopMonitoringPrecision: getIntegerFromEnvironmentVariable(
                'CLAIRDASH_EVENT_LOOP_MONITORING_PRECISION',
            ),
            labels: getObjectFromEnvironmentVariable(
                'CLAIRDASH_PROMETHEUS_LABELS',
            ),
        },
        allowMultiOrgs: process.env.ALLOW_MULTIPLE_ORGS === 'true',
        maxPayloadSize: process.env.CLAIRDASH_MAX_PAYLOAD || '5mb',
        query: {
            maxLimit:
                getIntegerFromEnvironmentVariable(
                    'CLAIRDASH_QUERY_MAX_LIMIT',
                ) || 5000,
            csvCellsLimit:
                getIntegerFromEnvironmentVariable(
                    'CLAIRDASH_CSV_CELLS_LIMIT',
                ) || 100000,
            timezone: process.env.CLAIRDASH_QUERY_TIMEZONE,
        },
        chart: {
            versionHistory: {
                daysLimit:
                    getIntegerFromEnvironmentVariable(
                        'CLAIRDASH_CHART_VERSION_HISTORY_DAYS_LIMIT',
                    ) || 3,
            },
        },
        customVisualizations: {
        ...
        pivotTable: {
            maxColumnLimit:
                getIntegerFromEnvironmentVariable(
                    'CLAIRDASH_PIVOT_TABLE_MAX_COLUMN_LIMIT',
                ) || 60,
        },
        s3: {
        ...
        headlessBrowser: {
            ...
            internalClairdashHost:
                process.env.INTERNAL_CLAIRDASH_HOST || siteUrl,
        },
        resultsCache: {
        ...
        logging: {
            level: parseLoggingLevel(
                process.env.CLAIRDASH_LOG_LEVEL ||
                    ((process.env.NODE_ENV || 'development') === 'development'
                        ? 'debug'
                        : 'http'),
            ),
            format: parseLoggingFormat(
                process.env.CLAIRDASH_LOG_FORMAT || 'pretty',
            ),
            outputs: (process.env.CLAIRDASH_LOG_OUTPUTS
                ? process.env.CLAIRDASH_LOG_OUTPUTS.split(',')
                : ['console']
            ).map(parseLoggingOutput),
            consoleFormat:
                process.env.CLAIRDASH_LOG_CONSOLE_FORMAT === undefined
                    ? undefined
                    : parseLoggingFormat(
                          process.env.CLAIRDASH_LOG_CONSOLE_FORMAT,
                      ),
            consoleLevel:
                process.env.CLAIRDASH_LOG_CONSOLE_LEVEL === undefined
                    ? undefined
                    : parseLoggingLevel(
                          process.env.CLAIRDASH_LOG_CONSOLE_LEVEL,
                      ),
            fileFormat:
                process.env.CLAIRDASH_LOG_FILE_FORMAT === undefined
                    ? undefined
                    : parseLoggingFormat(process.env.CLAIRDASH_LOG_FILE_FORMAT),
            fileLevel:
                process.env.CLAIRDASH_LOG_FILE_LEVEL === undefined
                    ? undefined
                    : parseLoggingLevel(process.env.CLAIRDASH_LOG_FILE_LEVEL),
            filePath: process.env.CLAIRDASH_LOG_FILE_PATH || './logs/all.log',
        },
        github: {
            appName: process.env.GITHUB_APP_NAME || 'clairdash-app-dev',
            redirectDomain:

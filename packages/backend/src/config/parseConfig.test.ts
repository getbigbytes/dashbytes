...
import { ORGANIZATION_ADMIN } from '@bigbytes/common/dist/authorization/organizationMemberAbility.mock';
...
beforeEach(() => {
    process.env = {
        BIGBYTES_SECRET: 'not very secret',
    };
});
...
test('Should use default sentry configuration if no environment vars', () => {
    const expected: SentryConfig = {
        ...
        release: VERSION,
        environment: BigbytesMode.DEFAULT,
...
test('Should throw error when secret missing', () => {
    delete process.env.BIGBYTES_SECRET;
    expect(() => parseConfig()).toThrowError(ParseError);
});

test('Should include secret in output', () => {
    process.env.BIGBYTES_SECRET = 'so very secret';
    expect(parseConfig().bigbytesSecret).toEqual('so very secret');
});

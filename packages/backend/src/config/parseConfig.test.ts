...
import { ORGANIZATION_ADMIN } from '@clairdash/common/dist/authorization/organizationMemberAbility.mock';
...
beforeEach(() => {
    process.env = {
        CLAIRDASH_SECRET: 'not very secret',
    };
});
...
test('Should use default sentry configuration if no environment vars', () => {
    const expected: SentryConfig = {
        ...
        release: VERSION,
        environment: ClairdashMode.DEFAULT,
...
test('Should throw error when secret missing', () => {
    delete process.env.CLAIRDASH_SECRET;
    expect(() => parseConfig()).toThrowError(ParseError);
});

test('Should include secret in output', () => {
    process.env.CLAIRDASH_SECRET = 'so very secret';
    expect(parseConfig().clairdashSecret).toEqual('so very secret');
});

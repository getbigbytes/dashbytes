...
const setupOktaIssuerClient = async () => {
    const { okta } = bigbytesConfig.auth;
    ...
    const redirectUri = new URL(
        `/api/v1${bigbytesConfig.auth.okta.callbackPath}`,
            ...
            if (req.query.error) {
                if (req.query.error === 'access_denied') {
                    return this.fail(
                        {
                            message:
                                'Your Okta account doesn’t currently have access to Bigbytes. Please contact support or your Okta administrator to enable access.',
                        },
                        401,
                    );
                }
    ...
    try {
        ...
        const authorizationUrl = client.authorizationUrl({
            ...
            scope: 'openid profile email'.concat(
                bigbytesConfig.auth.okta.extraScopes
                    ? ` ${bigbytesConfig.auth.okta.extraScopes}`
                    : '',
            ),
...
export const isOktaPassportStrategyAvailableToUse = !!(
    bigbytesConfig.auth.okta.oauth2ClientId &&
    bigbytesConfig.auth.okta.oauth2ClientSecret &&
    bigbytesConfig.auth.okta.oauth2Issuer &&
    bigbytesConfig.auth.okta.oktaDomain
);

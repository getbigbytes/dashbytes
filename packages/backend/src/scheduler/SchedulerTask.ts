...
import {
    ...
    BigbytesAnalytics,
    parseAnalyticsLimit,
} from '../analytics/BigbytesAnalytics';
...
export default class SchedulerTask {
    protected readonly bigbytesConfig: BigbytesConfig;

    protected readonly analytics: BigbytesAnalytics;
    ...
    protected async getChartOrDashboard(
        ...
        if (chartUuid) {
            ...
            return {
                url: `${this.bigbytesConfig.siteUrl}/projects/${chart.projectUuid}/saved/${chartUuid}`,
                minimalUrl: `${this.bigbytesConfig.headlessBrowser.internalBigbytesHost}/minimal/projects/${chart.projectUuid}/saved/${chartUuid}?context=${context}`,
                details: {
                    name: chart.name,
                    description: chart.description,
                },
                pageType: BigbytesPage.CHART,
        ...
        if (dashboardUuid) {
            ...
            return {
                url: `${this.bigbytesConfig.siteUrl}/projects/${dashboard.projectUuid}/dashboards/${dashboardUuid}/view`,
                minimalUrl: `${
    ...
    protected async sendSlackNotification(
        ...
        this.analytics.track({
            ...
            anonymousId: BigbytesAnalytics.anonymousId,
            properties: {
                jobId,
            ...
            if (thresholds !== undefined && thresholds.length > 0) {
                ...
                if (savedChartUuid) {
                    ...
                    const blocks = getChartThresholdAlertBlocks({
                        ...getBlocksArgs,
                        footerMarkdown: `This is a ${thresholdFooter} sent by Bigbytes. For security reasons, delivered files expire after ${
        ...
        try {
            ...
            if (thresholds !== undefined && thresholds.length > 0) {
                ...
                await this.emailClient.sendImageNotificationEmail(
                    ...
                    'This is a data alert sent by Bigbytes',
                );
            } else if (format === SchedulerFormat.IMAGE) {
                if (imageUrl === undefined) {
                    throw new Error('Missing image URL');
    ...
    protected async uploadGsheets(
        ...
        try {
            ...
            if (format !== SchedulerFormat.GSHEETS) {
                ...
                const reportUrl = `${this.bigbytesConfig.siteUrl}/projects/${chart.projectUuid}/saved/${chart.uuid}/view?scheduler_uuid=${schedulerUuid}&isSync=true`;

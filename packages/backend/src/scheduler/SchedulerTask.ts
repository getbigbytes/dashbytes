...
import {
    ...
    ClairdashAnalytics,
    parseAnalyticsLimit,
} from '../analytics/ClairdashAnalytics';
...
export default class SchedulerTask {
    protected readonly clairdashConfig: ClairdashConfig;

    protected readonly analytics: ClairdashAnalytics;
    ...
    protected async getChartOrDashboard(
        ...
        if (chartUuid) {
            ...
            return {
                url: `${this.clairdashConfig.siteUrl}/projects/${chart.projectUuid}/saved/${chartUuid}`,
                minimalUrl: `${this.clairdashConfig.headlessBrowser.internalClairdashHost}/minimal/projects/${chart.projectUuid}/saved/${chartUuid}?context=${context}`,
                details: {
                    name: chart.name,
                    description: chart.description,
                },
                pageType: ClairdashPage.CHART,
        ...
        if (dashboardUuid) {
            ...
            return {
                url: `${this.clairdashConfig.siteUrl}/projects/${dashboard.projectUuid}/dashboards/${dashboardUuid}/view`,
                minimalUrl: `${
    ...
    protected async sendSlackNotification(
        ...
        this.analytics.track({
            ...
            anonymousId: ClairdashAnalytics.anonymousId,
            properties: {
                jobId,
            ...
            if (thresholds !== undefined && thresholds.length > 0) {
                ...
                if (savedChartUuid) {
                    ...
                    const blocks = getChartThresholdAlertBlocks({
                        ...getBlocksArgs,
                        footerMarkdown: `This is a ${thresholdFooter} sent by Clairdash. For security reasons, delivered files expire after ${
        ...
        try {
            ...
            if (thresholds !== undefined && thresholds.length > 0) {
                ...
                await this.emailClient.sendImageNotificationEmail(
                    ...
                    'This is a data alert sent by Clairdash',
                );
            } else if (format === SchedulerFormat.IMAGE) {
                if (imageUrl === undefined) {
                    throw new Error('Missing image URL');
    ...
    protected async uploadGsheets(
        ...
        try {
            ...
            if (format !== SchedulerFormat.GSHEETS) {
                ...
                const reportUrl = `${this.clairdashConfig.siteUrl}/projects/${chart.projectUuid}/saved/${chart.uuid}/view?scheduler_uuid=${schedulerUuid}&isSync=true`;

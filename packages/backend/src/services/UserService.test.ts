import { EmailStatus, OpenIdIdentityIssuerType } from '@clairdash/common';
import { analyticsMock } from '../analytics/ClairdashAnalytics.mock';
...
const createUserService = (clairdashConfig: ClairdashConfig) =>
    new UserService({
...
describe('UserService', () => {
    const userService = createUserService(clairdashConfigMock);
    ...
    test('should previous logged in sso provider', async () => {
        ...
        expect(await service.getLoginOptions('test@digi-trans.org')).toEqual({
            forceRedirect: true,
            redirectUri:
                'https://test.clairdash.cloud/api/v1/login/okta?login_hint=test%40clairdash.com',
            showOptions: ['okta'],
        });
    });
    ...
    test('should not redirect if only 1 sso is available but no email match', async () => {
        const service = createUserService({
            ...
            auth: {
                ...
                disablePasswordAuthentication: true,
                google: {
                    ...clairdashConfigMock.auth.google,
                    enabled: true,
    ...
    test('should return all available sso providers and email', async () => {
        const service = createUserService({
            ...
            auth: {
                ...
                oneLogin: {
                    ...clairdashConfigMock.auth.oneLogin,
                    oauth2ClientId: '1',
                    loginPath: '/login/oneLogin',
                },
                azuread: {
                    ...clairdashConfigMock.auth.azuread,
                    oauth2ClientId: '1',

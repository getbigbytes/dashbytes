import { EmailStatus, OpenIdIdentityIssuerType } from '@bigbytes/common';
import { analyticsMock } from '../analytics/BigbytesAnalytics.mock';
...
const createUserService = (bigbytesConfig: BigbytesConfig) =>
    new UserService({
...
describe('UserService', () => {
    const userService = createUserService(bigbytesConfigMock);
    ...
    test('should not redirect if only 1 sso is available but no email match', async () => {
        const service = createUserService({
            ...
            auth: {
                ...
                disablePasswordAuthentication: true,
                google: {
                    ...bigbytesConfigMock.auth.google,
                    enabled: true,
    ...
    test('should return all available sso providers and email', async () => {
        const service = createUserService({
            ...
            auth: {
                ...
                oneLogin: {
                    ...bigbytesConfigMock.auth.oneLogin,
                    oauth2ClientId: '1',
                    loginPath: '/login/oneLogin',
                },
                azuread: {
                    ...bigbytesConfigMock.auth.azuread,
                    oauth2ClientId: '1',

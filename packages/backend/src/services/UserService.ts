...
export class UserService extends BaseService {
    ...
    private identifyUser(
        user: ClairdashUser & { isMarketingOptedIn?: boolean },
    ): void {
        if (this.clairdashConfig.mode === ClairdashMode.DEMO) {
            return;
        }
    ...
    async checkNewUserRegistrationAllowed(
        ...
        if (
            ...
            !this.clairdashConfig.allowMultiOrgs &&
    ...
    async loginWithOpenId(
        ...
        if (openIdSession) {
            ...
            if (
                this.clairdashConfig.groups.enabled === true &&
                this.clairdashConfig.auth.enableGroupSync === true &&
        ...
        if (!authenticatedUser && this.clairdashConfig.auth.enableOidcLinking) {
            const identities =
        ...
        if (
            !authenticatedUser &&
            this.clairdashConfig.auth.enableOidcToEmailLinking
        ) {
    ...
    private async linkOpenIdIdentityToUser(
        ...
        if (
            ...
            this.clairdashConfig.auth.enableGroupSync &&
            Array.isArray(openIdUser.openId.groups) &&
            openIdUser.openId.groups.length &&
            sessionUser.organizationUuid
        ) {
    ...
    async loginWithPassword(
        ...
        try {
            if (this.clairdashConfig.auth.disablePasswordAuthentication) {
                throw new ForbiddenError(
    ...
    async registerOrActivateUser(
        ...
        if (hasInviteCode(user)) {
            clairdashUser = await this.activateUserFromInvite(user.inviteCode, {
            ...
            clairdashUser = await this.registerUser({
                firstName: user.firstName,
                lastName: user.lastName,
                email: user.email,
                password: user.password,
        ...
        return this.userModel.findSessionUserByUUID(clairdashUser.userUuid);
    }
    ...
    async sendOneTimePasscodeToPrimaryEmail(
        ...
        const passcode =
            this.clairdashConfig.mode === ClairdashMode.PR ||
            this.clairdashConfig.mode === ClairdashMode.DEV
                ? '000000'
    ...
    async isLoginMethodAllowed(_email: string, loginMethod: LoginOptionTypes) {
        switch (loginMethod) {
            case LocalIssuerTypes.EMAIL:
                return !this.clairdashConfig.auth.disablePasswordAuthentication;
    ...
    private getRedirectUri(issuer: OpenIdIdentityIssuerType) {
        switch (issuer) {
            case OpenIdIdentityIssuerType.AZUREAD:
                return this.clairdashConfig.auth.azuread.loginPath;
            case OpenIdIdentityIssuerType.GOOGLE:
                return this.clairdashConfig.auth.google.loginPath;
            case OpenIdIdentityIssuerType.OKTA:
                return this.clairdashConfig.auth.okta.loginPath;
            case OpenIdIdentityIssuerType.ONELOGIN:
                return this.clairdashConfig.auth.oneLogin.loginPath;
            case OpenIdIdentityIssuerType.GENERIC_OIDC:
                return this.clairdashConfig.auth.oidc.loginPath;
            default:
                assertUnreachable(
                    issuer,
    ...
    private getInstanceLoginOptions() {
        ...
        if (this.clairdashConfig.auth.google.enabled) {
            options.add(OpenIdIdentityIssuerType.GOOGLE);
        }
        if (this.clairdashConfig.auth.azuread?.oauth2ClientId !== undefined) {
            options.add(OpenIdIdentityIssuerType.AZUREAD);
        }
        if (this.clairdashConfig.auth.oneLogin?.oauth2ClientId) {
            options.add(OpenIdIdentityIssuerType.ONELOGIN);
        }
        if (this.clairdashConfig.auth.okta?.oauth2ClientId) {
            options.add(OpenIdIdentityIssuerType.OKTA);
        }
        if (this.clairdashConfig.auth.oidc.clientId) {

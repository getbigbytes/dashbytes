...
export class UserService extends BaseService {
    ...
    private identifyUser(
        user: BigbytesUser & { isMarketingOptedIn?: boolean },
    ): void {
        if (this.bigbytesConfig.mode === BigbytesMode.DEMO) {
            return;
        }
    ...
    async checkNewUserRegistrationAllowed(
        ...
        if (
            ...
            !this.bigbytesConfig.allowMultiOrgs &&
    ...
    async loginWithOpenId(
        ...
        if (openIdSession) {
            ...
            if (
                this.bigbytesConfig.groups.enabled === true &&
                this.bigbytesConfig.auth.enableGroupSync === true &&
        ...
        if (!authenticatedUser && this.bigbytesConfig.auth.enableOidcLinking) {
            const identities =
        ...
        if (
            !authenticatedUser &&
            this.bigbytesConfig.auth.enableOidcToEmailLinking
        ) {
    ...
    private async linkOpenIdIdentityToUser(
        ...
        if (
            ...
            this.bigbytesConfig.auth.enableGroupSync &&
            Array.isArray(openIdUser.openId.groups) &&
            openIdUser.openId.groups.length &&
            sessionUser.organizationUuid
        ) {
    ...
    async loginWithPassword(
        ...
        try {
            if (this.bigbytesConfig.auth.disablePasswordAuthentication) {
                throw new ForbiddenError(
    ...
    async registerOrActivateUser(
        ...
        if (hasInviteCode(user)) {
            bigbytesUser = await this.activateUserFromInvite(user.inviteCode, {
            ...
            bigbytesUser = await this.registerUser({
                firstName: user.firstName,
                lastName: user.lastName,
                email: user.email,
                password: user.password,
        ...
        return this.userModel.findSessionUserByUUID(bigbytesUser.userUuid);
    }
    ...
    async sendOneTimePasscodeToPrimaryEmail(
        ...
        const passcode =
            this.bigbytesConfig.mode === BigbytesMode.PR ||
            this.bigbytesConfig.mode === BigbytesMode.DEV
                ? '000000'
    ...
    async isLoginMethodAllowed(_email: string, loginMethod: LoginOptionTypes) {
        switch (loginMethod) {
            case LocalIssuerTypes.EMAIL:
                return !this.bigbytesConfig.auth.disablePasswordAuthentication;
    ...
    private getRedirectUri(issuer: OpenIdIdentityIssuerType) {
        switch (issuer) {
            case OpenIdIdentityIssuerType.AZUREAD:
                return this.bigbytesConfig.auth.azuread.loginPath;
            case OpenIdIdentityIssuerType.GOOGLE:
                return this.bigbytesConfig.auth.google.loginPath;
            case OpenIdIdentityIssuerType.OKTA:
                return this.bigbytesConfig.auth.okta.loginPath;
            case OpenIdIdentityIssuerType.ONELOGIN:
                return this.bigbytesConfig.auth.oneLogin.loginPath;
            case OpenIdIdentityIssuerType.GENERIC_OIDC:
                return this.bigbytesConfig.auth.oidc.loginPath;
            default:
                assertUnreachable(
                    issuer,
    ...
    private getInstanceLoginOptions() {
        ...
        if (this.bigbytesConfig.auth.google.enabled) {
            options.add(OpenIdIdentityIssuerType.GOOGLE);
        }
        if (this.bigbytesConfig.auth.azuread?.oauth2ClientId !== undefined) {
            options.add(OpenIdIdentityIssuerType.AZUREAD);
        }
        if (this.bigbytesConfig.auth.oneLogin?.oauth2ClientId) {
            options.add(OpenIdIdentityIssuerType.ONELOGIN);
        }
        if (this.bigbytesConfig.auth.okta?.oauth2ClientId) {
            options.add(OpenIdIdentityIssuerType.OKTA);
        }
        if (this.bigbytesConfig.auth.oidc.clientId) {

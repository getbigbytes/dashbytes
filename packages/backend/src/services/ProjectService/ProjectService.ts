...
import { SshTunnel } from '@bigbytes/warehouses';
...
export class ProjectService extends BaseService {
    ...
    async compileQuery(
        ...
        const compiledQuery = await ProjectService._compileQuery(
            ...
            intrinsicUserAttributes,
            userAttributes,
            this.bigbytesConfig.query.timezone || 'UTC',
        );
        await sshTunnel.disconnect();
    ...
    private metricQueryWithLimit(
        ...
        if (csvLimit === undefined) {
            if (metricQuery.limit > this.bigbytesConfig.query?.maxLimit) {
                throw new ParameterError(
                    `Query limit can not exceed ${this.bigbytesConfig.query.maxLimit}`,
                );
            }
            return metricQuery;
        }
        ...
        const cellsLimit = this.bigbytesConfig.query?.csvCellsLimit || 100000;
    ...
    private async getResultsFromCacheOrWarehouse({
        ...
        return wrapSentryTransaction(
            ...
            async (span) => {
                ...
                if (
                    this.bigbytesConfig.resultsCache?.enabled &&
                    !invalidateCache
                ) {
                    ...
                    if (
                        ...
                        new Date().getTime() -
                            ...
                            this.bigbytesConfig.resultsCache
                ...
                if (this.bigbytesConfig.resultsCache?.enabled) {
                    this.logger.debug(
        ...
        return wrapSentryTransaction(
            ...
            async (span) => {
                try {
                    ...
                    span.setAttribute('bigbytes.projectUuid', projectUuid);
                    span.setAttribute(
                        'warehouse.type',
    ...
    async runSqlQuery(
        ...
        const cteWithLimit = applyLimitToSqlQuery({
            sqlQuery: sql,
            limit: this.bigbytesConfig.query.maxLimit,
        });
    ...
    async streamSqlQueryIntoFile({
        ...
        const fileUrl = await this.downloadFileModel.streamFunction(
            ...
            `${this.bigbytesConfig.siteUrl}/api/v1/projects/${projectUuid}/sqlRunner/results`,
            async (writer) => {
    ...
    async searchFieldUniqueValues(
        ...
        if (limit > this.bigbytesConfig.query.maxLimit) {
            throw new ParameterError(
                `Query limit can not exceed ${this.bigbytesConfig.query.maxLimit}`,
            );
        }
    ...
    */
    ...
    async getDbtExposures(
        ...
        const chartExposures = charts.reduce<DbtExposure[]>((acc, chart) => {
            ...
            if (dependsOn.length > 0) {
                acc.push({
                    ...
                    url: `${this.bigbytesConfig.siteUrl}/projects/${projectUuid}/saved/${chart.uuid}/view`,
                    dependsOn,
                    tags: ['bigbytes', 'chart'],
                });
            }
        ...
        const dashboardExposures = dashboards.reduce<DbtExposure[]>(
            (acc, dashboard) => {
                acc.push({
                    ...
                    url: `${this.bigbytesConfig.siteUrl}/projects/${projectUuid}/dashboards/${dashboard.uuid}/view`,
        ...
        const projectExposure: DbtExposure = {
            ...
            label: `Bigbytes - ${projectSummary.name}`,
            description: 'Bigbytes project',
            url: `${this.bigbytesConfig.siteUrl}/projects/${projectUuid}/home`,
            dependsOn: uniq(
                chartExposures.map(({ dependsOn }) => dependsOn).flat(),
            ),
            tags: ['bigbytes', 'project'],
        };
        ...
        return charts.reduce<any[]>((acc, chart) => {
            ...
            const metrics = [
                ...
                ...customMetrics.map((metric) => ({
                    ...
                    chartUrl: `${this.bigbytesConfig.siteUrl}/projects/${projectUuid}/saved/${chart.uuid}/view`,
                })),
            ];

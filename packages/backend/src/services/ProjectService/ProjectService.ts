...
import { SshTunnel } from '@clairdash/warehouses';
...
export class ProjectService extends BaseService {
    ...
    async compileQuery(
        ...
        const compiledQuery = await ProjectService._compileQuery(
            ...
            intrinsicUserAttributes,
            userAttributes,
            this.clairdashConfig.query.timezone || 'UTC',
        );
        await sshTunnel.disconnect();
    ...
    private metricQueryWithLimit(
        ...
        if (csvLimit === undefined) {
            if (metricQuery.limit > this.clairdashConfig.query?.maxLimit) {
                throw new ParameterError(
                    `Query limit can not exceed ${this.clairdashConfig.query.maxLimit}`,
                );
            }
            return metricQuery;
        }
        ...
        const cellsLimit = this.clairdashConfig.query?.csvCellsLimit || 100000;
    ...
    private async getResultsFromCacheOrWarehouse({
        ...
        return wrapSentryTransaction(
            ...
            async (span) => {
                ...
                if (
                    this.clairdashConfig.resultsCache?.enabled &&
                    !invalidateCache
                ) {
                    ...
                    if (
                        ...
                        new Date().getTime() -
                            ...
                            this.clairdashConfig.resultsCache
                ...
                if (this.clairdashConfig.resultsCache?.enabled) {
                    this.logger.debug(
        ...
        return wrapSentryTransaction(
            ...
            async (span) => {
                try {
                    ...
                    span.setAttribute('clairdash.projectUuid', projectUuid);
                    span.setAttribute(
                        'warehouse.type',
    ...
    async runSqlQuery(
        ...
        const cteWithLimit = applyLimitToSqlQuery({
            sqlQuery: sql,
            limit: this.clairdashConfig.query.maxLimit,
        });
    ...
    async streamSqlQueryIntoFile({
        ...
        const fileUrl = await this.downloadFileModel.streamFunction(
            ...
            `${this.clairdashConfig.siteUrl}/api/v1/projects/${projectUuid}/sqlRunner/results`,
            async (writer) => {
    ...
    async searchFieldUniqueValues(
        ...
        if (limit > this.clairdashConfig.query.maxLimit) {
            throw new ParameterError(
                `Query limit can not exceed ${this.clairdashConfig.query.maxLimit}`,
            );
        }
    ...
    */
    ...
    async getDbtExposures(
        ...
        const chartExposures = charts.reduce<DbtExposure[]>((acc, chart) => {
            ...
            if (dependsOn.length > 0) {
                acc.push({
                    ...
                    url: `${this.clairdashConfig.siteUrl}/projects/${projectUuid}/saved/${chart.uuid}/view`,
                    dependsOn,
                    tags: ['clairdash', 'chart'],
                });
            }
            return acc;
        }, []);
        ...
        const dashboardExposures = dashboards.reduce<DbtExposure[]>(
            (acc, dashboard) => {
                acc.push({
                    ...
                    url: `${this.clairdashConfig.siteUrl}/projects/${projectUuid}/dashboards/${dashboard.uuid}/view`,
        ...
        const projectExposure: DbtExposure = {
            ...
            label: `Clairdash - ${projectSummary.name}`,
            description: 'Clairdash project',
            url: `${this.clairdashConfig.siteUrl}/projects/${projectUuid}/home`,
            dependsOn: uniq(
                chartExposures.map(({ dependsOn }) => dependsOn).flat(),
            ),
            tags: ['clairdash', 'project'],
        };
        ...
        return charts.reduce<any[]>((acc, chart) => {
            ...
            const metrics = [
                ...
                ...customMetrics.map((metric) => ({
                    ...
                    chartUrl: `${this.clairdashConfig.siteUrl}/projects/${projectUuid}/saved/${chart.uuid}/view`,
                })),
            ];

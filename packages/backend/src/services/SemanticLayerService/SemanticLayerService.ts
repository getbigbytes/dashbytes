...
export class SemanticLayerService extends BaseService {
    ...
    private validateQueryLimit(query: SemanticLayerQuery) {
        if (query.limit && query.limit > this.bigbytesConfig.query.maxLimit) {
            throw new ParameterError(
                `Limit cannot be greater than ${this.bigbytesConfig.query.maxLimit}`,
            );
        }
    }
    ...
    async streamQueryIntoFile({
        ...
        const fileUrl = await this.downloadFileModel.streamFunction(
            ...
            `${this.bigbytesConfig.siteUrl}/api/v2/projects/${projectUuid}/semantic-layer/results`,

...
export class UnfurlService extends BaseService {
    ...
    > {
        switch (parsedUrl.bigbytesPage) {
            case BigbytesPage.DASHBOARD:
            ...
            default:
                return assertUnreachable(
                    parsedUrl.bigbytesPage,
                    `No bigbytes page Slack unfurl implemented`,
                );
        }
    }
    ...
    async unfurlDetails(originUrl: string): Promise<Unfurl | undefined> {
        ...
        if (
            !parsedUrl.isValid ||
            parsedUrl.bigbytesPage === undefined ||
        ...
        return {
            title,
            description,
            pageType: parsedUrl.bigbytesPage,
            imageUrl: undefined,
    ...
    async unfurlImage({
        ...
        if (buffer !== undefined) {
            ...
            if (this.s3Client.isEnabled()) {
                ...
                // We will share the image saved by puppetteer on our bigbytes enpdoint
    ...
    async exportDashboard(
        ...
        const { organizationUuid, projectUuid, name, minimalUrl, pageType } = {
            ...
            minimalUrl: new URL(
                ...
                this.bigbytesConfig.headlessBrowser.internalBigbytesHost,
            ).href,
            pageType: BigbytesPage.DASHBOARD,
        };
    ...
    private async saveScreenshot({
        ...
        if (this.bigbytesConfig.headlessBrowser?.host === undefined) {
            this.logger.error(
            ...
            async (span) => {
                ...
                try {
                    const browserWSEndpoint = `ws://${
                        this.bigbytesConfig.headlessBrowser?.host
                    }:${this.bigbytesConfig.headlessBrowser?.port || 3001}`;
                    ...
                    page.on('response', async (response) => {
                        ...
                        const regexUrlToMatch =
                            bigbytesPage === BigbytesPage.EXPLORE ||
                            bigbytesPage === BigbytesPage.CHART
                    ...
                    try {
                        ...
                        if (bigbytesPage === BigbytesPage.DASHBOARD) {
                        ...
                        } else if (bigbytesPage === BigbytesPage.CHART) {
                        ...
                        } else if (bigbytesPage === BigbytesPage.EXPLORE) {
                    ...
                    if (bigbytesPage === BigbytesPage.EXPLORE) {
                        finalSelector = `[data-testid="visualization"]`;
                    } else if (bigbytesPage === BigbytesPage.DASHBOARD) {
                    ...
                    if (
                        bigbytesPage === BigbytesPage.DASHBOARD ||
                        bigbytesPage === BigbytesPage.EXPLORE
                    ) {
                    ...
                    if (isRetryableError && retries) {
                        this.logger.info(
                            `Retrying: unable to fetch screenshots for scheduler with url ${url}, of type: ${bigbytesPage}. Message: ${e.message}`,
                        );
                        span.addEvent(e);
                        span.setAttributes({
                            'page.type': bigbytesPage,
                    ...
                    this.logger.error(
                        `Unable to fetch screenshots for scheduler with url ${url}, of type: ${bigbytesPage}. Message: ${e.message}`,
                    );
                    throw e;
                } finally {

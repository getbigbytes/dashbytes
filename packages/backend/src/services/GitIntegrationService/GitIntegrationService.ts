...
export class GitIntegrationService extends BaseService {
    ...
    async getPullRequestDetails({
        ...
        const prBody = `Created by Clairdash, this PR adds ${customMetrics.length} custom metrics to the dbt model
...
- [${chart.name}](${
    ...
    private static getFilePath(
        ...
        const filePath = `${path}/models/clairdash/${snakeCaseName(
            name,
        )}.${extension}`;
...
`;
    ...
    async createPullRequestFromSql(
        ...
        const branchName = `clairdash-${userName}-${nanoid(4)}`;
        ...
        const pullRequest = await createPullRequest({
            ...
            body: `Created by Clairdash, this pull request introduces a new SQL file and a corresponding Clairdash \`.yml\` configuration file.

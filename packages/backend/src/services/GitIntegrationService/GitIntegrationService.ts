...
export class GitIntegrationService extends BaseService {
    ...
    async getPullRequestDetails({
        ...
        const prBody = `Created by Bigbytes, this PR adds ${customMetrics.length} custom metrics to the dbt model
...
- [${chart.name}](${
    ...
    private static getFilePath(
        ...
        const filePath = `${path}/models/bigbytes/${snakeCaseName(
            name,
        )}.${extension}`;
...
`;
    ...
    async createPullRequestFromSql(
        ...
        const branchName = `bigbytes-${userName}-${nanoid(4)}`;
        ...
        const pullRequest = await createPullRequest({
            ...
            body: `Created by Bigbytes, this pull request introduces a new SQL file and a corresponding Bigbytes \`.yml\` configuration file.

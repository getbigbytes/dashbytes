...
apiV1Router.post('/login', passport.authenticate('local'), (req, res, next) => {
    req.session.save((err) => {
        if (err) {
            ...
            res.json({
                status: 'ok',
                results: UserModel.clairdashUserFromSession(req.user!),
            });
        }
    });
});
...
apiV1Router.get(
    clairdashConfig.auth.okta.loginPath,
    storeOIDCRedirect,
    initiateOktaOpenIdLogin,
);

apiV1Router.get(clairdashConfig.auth.okta.callbackPath, (req, res, next) =>
...
apiV1Router.get(
    clairdashConfig.auth.azuread.loginPath,
    storeOIDCRedirect,
...
apiV1Router.get(clairdashConfig.auth.azuread.callbackPath, (req, res, next) =>
    passport.authenticate('azuread', {
        failureRedirect: getOidcRedirectURL(false)(req),
        successRedirect: getOidcRedirectURL(true)(req),
        failureFlash: true,
...
apiV1Router.get(
    clairdashConfig.auth.oidc.loginPath,
    storeOIDCRedirect,
    passport.authenticate(
        'oidc',
        clairdashConfig.auth.oidc.scopes
            ? {
                  scope: clairdashConfig.auth.oidc.scopes,
              }
            : {},
    ),
);

apiV1Router.get(clairdashConfig.auth.oidc.callbackPath, (req, res, next) =>
...
apiV1Router.get(
    clairdashConfig.auth.oneLogin.loginPath,
    storeOIDCRedirect,
...
apiV1Router.get(clairdashConfig.auth.oneLogin.callbackPath, (req, res, next) =>
    passport.authenticate('oneLogin', {
        failureRedirect: getOidcRedirectURL(false)(req),
        successRedirect: getOidcRedirectURL(true)(req),
        failureFlash: true,
...
apiV1Router.get(
    clairdashConfig.auth.google.loginPath,
    storeOIDCRedirect,
    (req, res, next) => {
...
apiV1Router.get(clairdashConfig.auth.google.callbackPath, (req, res, next) => {
    passport.authenticate('google', {
        failureRedirect: getOidcRedirectURL(false)(req),
        successRedirect: getOidcRedirectURL(true)(req),
        failureFlash: true,

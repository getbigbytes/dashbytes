import { ForbiddenError } from '@bigbytes/common';
...
export const getAuthenticationToken = (value: string) =>
    createHmac('sha512', bigbytesConfig.bigbytesSecret)
        .update(value)
        .digest('hex');
...
if (
    ...
    headlessBrowserRouter.get('/test/:flag', async (req, res, next) => {
        ...
        try {
            ...
            const testUrl = `${bigbytesConfig.siteUrl}/api/v1/headless-browser/callback/${req.params.flag}`;
    ...
    headlessBrowserRouter.get('/image/', async (req, res, next) => {
        ...
        try {
            ...
            if (bigbytesConfig.scheduler.screenshotTimeout) {
                await new Promise((resolve) => {
                    setTimeout(
                        resolve,
                        bigbytesConfig.scheduler.screenshotTimeout,
                    );
                });
            }

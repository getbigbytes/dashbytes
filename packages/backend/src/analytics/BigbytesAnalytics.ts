...
export type UpdateUserEvent = BaseTrack & {
    event: 'user.updated';
    userId?: string;
    properties: Omit<BigbytesUser, 'userUuid' | 'organizationUuid'> & {
...
export const getContextFromHeader = (req: Request) => {
    const method = getRequestMethod(req.header(BigbytesRequestMethodHeader));
    switch (method) {
...
type BigbytesAnalyticsArguments = {
...
export class BigbytesAnalytics extends Analytics {
    private readonly bigbytesConfig: BigbytesConfig;

    private readonly bigbytesContext: Record<string, any>;
    ...
    constructor({
        ...
        dataPlaneUrl,
        options,
    }: BigbytesAnalyticsArguments) {
        super(writeKey, dataPlaneUrl, options);
        this.bigbytesConfig = bigbytesConfig;
        this.bigbytesContext = {
            app: {
                namespace: 'bigbytes',
                name: 'bigbytes_server',
                version: VERSION,
                mode: bigbytesConfig.mode,
                siteUrl:
                    bigbytesConfig.mode === BigbytesMode.CLOUD_BETA
                        ? bigbytesConfig.siteUrl
                        : null,
                installId: process.env.BIGBYTES_INSTALL_ID || uuidv4(),
                installType:
                    process.env.BIGBYTES_INSTALL_TYPE ||
                    BigbytesInstallType.UNKNOWN,
            },
        };
    }

    static anonymousId = process.env.BIGBYTES_INSTALL_ID || uuidv4();

    identify(payload: Identify) {
        if (!this.bigbytesConfig.rudder.writeKey) return; // Tracking disabled

        super.identify({
            ...payload,
            context: { ...this.bigbytesContext }, // NOTE: spread because rudderstack manipulates arg
        });
    }
    ...
    track<T extends BaseTrack>(payload: TypedEvent | UntypedEvent<T>) {
        ...
        if (isUserUpdatedEvent(payload)) {
            ...
            super.track({
                ...payload,
                event: `${this.bigbytesContext.app.name}.${payload.event}`,
                context: { ...this.bigbytesContext }, // NOTE: spread because rudderstack manipulates arg

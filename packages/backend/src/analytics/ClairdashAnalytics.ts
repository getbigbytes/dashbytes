...
export type UpdateUserEvent = BaseTrack & {
    event: 'user.updated';
    userId?: string;
    properties: Omit<ClairdashUser, 'userUuid' | 'organizationUuid'> & {
...
export const getContextFromHeader = (req: Request) => {
    const method = getRequestMethod(req.header(ClairdashRequestMethodHeader));
    switch (method) {
...
type ClairdashAnalyticsArguments = {
...
export class ClairdashAnalytics extends Analytics {
    private readonly clairdashConfig: ClairdashConfig;

    private readonly clairdashContext: Record<string, any>;
    ...
    constructor({
        ...
        dataPlaneUrl,
        options,
    }: ClairdashAnalyticsArguments) {
        super(writeKey, dataPlaneUrl, options);
        this.clairdashConfig = clairdashConfig;
        this.clairdashContext = {
            app: {
                namespace: 'clairdash',
                name: 'clairdash_server',
                version: VERSION,
                mode: clairdashConfig.mode,
                siteUrl:
                    clairdashConfig.mode === ClairdashMode.CLOUD_BETA
                        ? clairdashConfig.siteUrl
                        : null,
                installId: process.env.CLAIRDASH_INSTALL_ID || uuidv4(),
                installType:
                    process.env.CLAIRDASH_INSTALL_TYPE ||
                    ClairdashInstallType.UNKNOWN,
            },
        };
    }

    static anonymousId = process.env.CLAIRDASH_INSTALL_ID || uuidv4();

    identify(payload: Identify) {
        if (!this.clairdashConfig.rudder.writeKey) return; // Tracking disabled

        super.identify({
            ...payload,
            context: { ...this.clairdashContext }, // NOTE: spread because rudderstack manipulates arg
        });
    }
    ...
    track<T extends BaseTrack>(payload: TypedEvent | UntypedEvent<T>) {
        ...
        if (isUserUpdatedEvent(payload)) {
            ...
            super.track({
                ...payload,
                event: `${this.clairdashContext.app.name}.${payload.event}`,
                context: { ...this.clairdashContext }, // NOTE: spread because rudderstack manipulates arg

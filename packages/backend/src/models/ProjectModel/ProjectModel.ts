...
export class ProjectModel {
    ...
    protected clairdashConfig: ClairdashConfig;
            ...
            if (dashboardTabs.length > 0) {
                newDashboardTabs = await trx(DashboardTabsTableName)
                    .insert(
                        dashboardTabs.map((d) => ({
                            ...d,
                            uuid: uuidv4(), // we need to generate the uuid here: https://github.com/clairview/clairdash/issues/10408
                            dashboard_id: dashboardMapping.find(
                                (m) => m.id === d.dashboard_id,
                            )?.newId!,

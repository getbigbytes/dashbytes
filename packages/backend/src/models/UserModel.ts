...
export const mapDbUserDetailsToClairdashUser = (
    user: DbUserDetails,
...
export class UserModel {
    ...
    constructor({ database, clairdashConfig }: UserModelArguments) {
        this.database = database;
        this.clairdashConfig = clairdashConfig;
    }

    private canTrackingBeAnonymized() {
        return this.clairdashConfig.mode !== ClairdashMode.CLOUD_BETA;
    }
    ...
    async getUserDetailsByUuid(userUuid: string): Promise<ClairdashUser> {
        ...
        return mapDbUserDetailsToClairdashUser(
            user,
            await this.hasAuthentication(userUuid),
        );
    }

    async getUserDetailsById(userId: number): Promise<ClairdashUser> {
        const [user] = await userDetailsQueryBuilder(this.database)
    ...
    async findSessionUserByOpenId(
        ...
        const clairdashUser = mapDbUserDetailsToClairdashUser(
            user,
            await this.hasAuthentication(user.user_uuid),
        );
        ...
        const abilityBuilder = getUserAbilityBuilder({
            ...
            permissionsConfig: {
                pat: this.clairdashConfig.auth.pat,
            },
        });
    ...
    static clairdashUserFromSession(
        sessionUser: SessionUser,
    ): ClairdashUserWithAbilityRules {
        const { userId, ability, ...clairdashUser } = sessionUser;
        return clairdashUser;
    }

    async findUserByEmail(email: string): Promise<ClairdashUser | undefined> {
        const [user] = await userDetailsQueryBuilder(this.database)
            .where('email', email)
            .select('*', 'organizations.created_at as organization_created_at');
        return user
            ? mapDbUserDetailsToClairdashUser(
                  user,
                  await this.hasAuthentication(user.user_uuid),
              )
            : undefined;

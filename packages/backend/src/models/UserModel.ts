...
export const mapDbUserDetailsToBigbytesUser = (
    user: DbUserDetails,
...
export class UserModel {
    ...
    constructor({ database, bigbytesConfig }: UserModelArguments) {
        this.database = database;
        this.bigbytesConfig = bigbytesConfig;
    }

    private canTrackingBeAnonymized() {
        return this.bigbytesConfig.mode !== BigbytesMode.CLOUD_BETA;
    }
    ...
    async getUserDetailsByUuid(userUuid: string): Promise<BigbytesUser> {
        ...
        return mapDbUserDetailsToBigbytesUser(
            user,
            await this.hasAuthentication(userUuid),
        );
    }

    async getUserDetailsById(userId: number): Promise<BigbytesUser> {
        const [user] = await userDetailsQueryBuilder(this.database)
    ...
    async findSessionUserByOpenId(
        ...
        const bigbytesUser = mapDbUserDetailsToBigbytesUser(
            user,
            await this.hasAuthentication(user.user_uuid),
        );
        ...
        const abilityBuilder = getUserAbilityBuilder({
            ...
            permissionsConfig: {
                pat: this.bigbytesConfig.auth.pat,
            },
        });
    ...
    static bigbytesUserFromSession(
        sessionUser: SessionUser,
    ): BigbytesUserWithAbilityRules {
        const { userId, ability, ...bigbytesUser } = sessionUser;
        return bigbytesUser;
    }

    async findUserByEmail(email: string): Promise<BigbytesUser | undefined> {
        const [user] = await userDetailsQueryBuilder(this.database)
            .where('email', email)
            .select('*', 'organizations.created_at as organization_created_at');
        return user
            ? mapDbUserDetailsToBigbytesUser(
                  user,
                  await this.hasAuthentication(user.user_uuid),
              )
            : undefined;

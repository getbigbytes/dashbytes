...
const schedulerWorkerFactory = (context: {
    ...
    new SchedulerWorker({
        bigbytesConfig: context.bigbytesConfig,
...
export default class App {
    ...
    async start() {
        ...
        Sentry.setTags({
            k8s_pod_name: this.bigbytesConfig.k8s.podName,
            k8s_pod_namespace: this.bigbytesConfig.k8s.podNamespace,
            k8s_node_name: this.bigbytesConfig.k8s.nodeName,
            bigbytes_cloud_instance:
                this.bigbytesConfig.bigbytesCloudInstance,
        });

        // Load Bigbytes middleware/routes last
        await this.initExpress(expressApp);

        if (this.bigbytesConfig.scheduler?.enabled) {
    ...
    private async initExpress(expressApp: Express) {
        ...
        expressApp.use(
            express.json({ limit: this.bigbytesConfig.maxPayloadSize }),
        );

        const reportUris: URL[] = [];
        try {
            if (this.bigbytesConfig.sentry.backend.securityReportUri) {
                const sentryReportUri = new URL(
                    this.bigbytesConfig.sentry.backend.securityReportUri,
                );
            ...
            if (this.bigbytesConfig.security.contentSecurityPolicy.reportUri) {
                reportUris.push(
                    new URL(
                        this.bigbytesConfig.security.contentSecurityPolicy.reportUri,
                    ),
                );
            }
        } catch (e) {
        ...
        const contentSecurityPolicyAllowedDomains: string[] = [
            ...
            'https://*.bigbytes.cloud',
            ...this.bigbytesConfig.security.contentSecurityPolicy
                .allowedDomains,
        ];
        ...
        expressApp.use(
            helmet({
                ...
                crossOriginOpenerPolicy: {
                    policy: [BigbytesMode.DEMO, BigbytesMode.PR].includes(
                        this.bigbytesConfig.mode,
                    )
                        ? 'unsafe-none'
                        : 'same-origin',
        ...
        expressApp.use(
            expressSession({
                secret: this.bigbytesConfig.bigbytesSecret,
                proxy: this.bigbytesConfig.trustProxy,
                rolling: true,
                cookie: {
                    maxAge:
                        (this.bigbytesConfig.cookiesMaxAgeHours || 24) *
                        60 *
                        60 *
                        1000, // in ms
                    secure: this.bigbytesConfig.secureCookies,
                    httpOnly: true,
                    sameSite: 'lax',
                },
                resave: false,
        ...
        expressApp.listen(this.port, () => {
            if (this.environment === 'production') {
                Logger.info(
                    `\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |  \n \\ | / \\ | / \\ | / \\ | / \\ | / \\ | / \\ | /\n  \\|/   \\|/   \\|/   \\|/   \\|/   \\|/   \\|/\n------------------------------------------\nLaunch bigbytes at http://localhost:${this.port}\n------------------------------------------\n  /|\\   /|\\   /|\\   /|\\   /|\\   /|\\   /|\\\n / | \\ / | \\ / | \\ / | \\ / | \\ / | \\ / | \\\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |`,
                );
            }
        });
        ...
        expressApp.use(
            (error: Error, req: Request, res: Response, _: NextFunction) => {
                ...
                if (
                    ...
                    !(error instanceof BigbytesError)
                ) {

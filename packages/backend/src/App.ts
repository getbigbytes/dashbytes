...
const schedulerWorkerFactory = (context: {
    ...
    new SchedulerWorker({
        clairdashConfig: context.clairdashConfig,
...
export default class App {
    ...
    async start() {
        ...
        Sentry.setTags({
            k8s_pod_name: this.clairdashConfig.k8s.podName,
            k8s_pod_namespace: this.clairdashConfig.k8s.podNamespace,
            k8s_node_name: this.clairdashConfig.k8s.nodeName,
            clairdash_cloud_instance:
                this.clairdashConfig.clairdashCloudInstance,
        });

        // Load Clairdash middleware/routes last
        await this.initExpress(expressApp);

        if (this.clairdashConfig.scheduler?.enabled) {
    ...
    private async initExpress(expressApp: Express) {
        ...
        expressApp.use(
            express.json({ limit: this.clairdashConfig.maxPayloadSize }),
        );

        const reportUris: URL[] = [];
        try {
            if (this.clairdashConfig.sentry.backend.securityReportUri) {
                const sentryReportUri = new URL(
                    this.clairdashConfig.sentry.backend.securityReportUri,
                );
            ...
            if (this.clairdashConfig.security.contentSecurityPolicy.reportUri) {
                reportUris.push(
                    new URL(
                        this.clairdashConfig.security.contentSecurityPolicy.reportUri,
                    ),
                );
            }
        } catch (e) {
        ...
        const contentSecurityPolicyAllowedDomains: string[] = [
            ...
            'https://*.clairdash.cloud',
            ...this.clairdashConfig.security.contentSecurityPolicy
                .allowedDomains,
        ];
        ...
        expressApp.use(
            helmet({
                ...
                crossOriginOpenerPolicy: {
                    policy: [ClairdashMode.DEMO, ClairdashMode.PR].includes(
                        this.clairdashConfig.mode,
                    )
                        ? 'unsafe-none'
                        : 'same-origin',
        ...
        expressApp.use(
            expressSession({
                secret: this.clairdashConfig.clairdashSecret,
                proxy: this.clairdashConfig.trustProxy,
                rolling: true,
                cookie: {
                    maxAge:
                        (this.clairdashConfig.cookiesMaxAgeHours || 24) *
                        60 *
                        60 *
                        1000, // in ms
                    secure: this.clairdashConfig.secureCookies,
                    httpOnly: true,
                    sameSite: 'lax',
                },
                resave: false,
        ...
        expressApp.listen(this.port, () => {
            if (this.environment === 'production') {
                Logger.info(
                    `\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |  \n \\ | / \\ | / \\ | / \\ | / \\ | / \\ | / \\ | /\n  \\|/   \\|/   \\|/   \\|/   \\|/   \\|/   \\|/\n------------------------------------------\nLaunch clairdash at http://localhost:${this.port}\n------------------------------------------\n  /|\\   /|\\   /|\\   /|\\   /|\\   /|\\   /|\\\n / | \\ / | \\ / | \\ / | \\ / | \\ / | \\ / | \\\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |\n   |     |     |     |     |     |     |`,
                );
            }
        });
        ...
        expressApp.use(
            (error: Error, req: Request, res: Response, _: NextFunction) => {
                ...
                if (
                    ...
                    !(error instanceof ClairdashError)
                ) {

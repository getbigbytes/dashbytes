    ...
    async getChannels(
        ...
        if (
            ...
            new Date().getTime() -
                ...
                (this.clairdashConfig.slack?.channelsCachedTime ||
                    DEFAULT_CACHE_TIME)
        ) {
    ...
    async updateAppCustomSettings(
        ...
        if (channelId && channelId !== currentChannelId) {
            ...
            await webClient.chat
                .postMessage({
                    channel: channelId,
                    text: `This channel will now receive notifications for failed scheduled delivery jobs in Clairdash. Configuration completed${

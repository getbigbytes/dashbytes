...
export class SlackBot {
    ...
    async start(expressApp: Express) {
        if (!this.clairdashConfig.slack?.clientId) {
            Logger.warn(`Missing "SLACK_CLIENT_ID", Slack App will not run`);
            return;
        }

        try {
            if (this.clairdashConfig.slack?.socketMode) {
                const app = new App({
                    ...slackOptions,
                    installationStore: {
                        storeInstallation: (i) =>
                    ...
                    port: this.clairdashConfig.slack.port,
                    socketMode: this.clairdashConfig.slack.socketMode,
                    appToken: this.clairdashConfig.slack.appToken,
                });
            ...
            try {
                ...
                if (details) {
                    ...
                    const { imageUrl } = await this.unfurlService.unfurlImage({
                        ...
                        clairdashPage: details.pageType,
                        imageId,
                        authUserUuid,
                    });
                ...
                if (this.clairdashConfig.mode === ClairdashMode.PR) {

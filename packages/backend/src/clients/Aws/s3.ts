...
import { MissingConfigError } from '@clairdash/common';
...
export class S3Client {
    ...
    constructor({ clairdashConfig }: S3ClientArguments) {
        this.clairdashConfig = clairdashConfig;

        if (clairdashConfig.s3?.endpoint && clairdashConfig.s3.region) {
            const s3Config = {
                region: clairdashConfig.s3.region,
                apiVersion: '2006-03-01',
                endpoint: clairdashConfig.s3.endpoint,
            };

            if (clairdashConfig.s3?.accessKey && clairdashConfig.s3.secretKey) {
                Object.assign(s3Config, {
                    credentials: {
                        accessKeyId: clairdashConfig.s3.accessKey,
                        secretAccessKey: clairdashConfig.s3.secretKey,
                    },
                });
    ...
    private async uploadFile(
        ...
        if (!this.clairdashConfig.s3?.bucket || this.s3 === undefined) {
        ...
        const upload = new Upload({
            client: this.s3,
            params: {
                Bucket: this.clairdashConfig.s3.bucket,
                Key: fileId,
                Body: file,
        ...
        try {
            ...
            const url = await getSignedUrl(
                ...
                {
                    expiresIn: this.clairdashConfig.s3.expirationTime,
                    ...urlOptions,
                },
            );
            return url;
            ...
            Logger.error(
                `Failed to upload file to s3 with endpoint: ${
                    this.clairdashConfig.s3.endpoint ?? 'no endpoint'
                }. ${error}`,
            );
    ...
    */
    ...
    getExpirationWarning() {
        if (this.isEnabled()) {
            const timeInSeconds =
                this.clairdashConfig.s3?.expirationTime || 259200;

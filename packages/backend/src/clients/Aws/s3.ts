...
import { MissingConfigError } from '@bigbytes/common';
...
export class S3Client {
    ...
    constructor({ bigbytesConfig }: S3ClientArguments) {
        this.bigbytesConfig = bigbytesConfig;

        if (bigbytesConfig.s3?.endpoint && bigbytesConfig.s3.region) {
            const s3Config = {
                region: bigbytesConfig.s3.region,
                apiVersion: '2006-03-01',
                endpoint: bigbytesConfig.s3.endpoint,
            };

            if (bigbytesConfig.s3?.accessKey && bigbytesConfig.s3.secretKey) {
                Object.assign(s3Config, {
                    credentials: {
                        accessKeyId: bigbytesConfig.s3.accessKey,
                        secretAccessKey: bigbytesConfig.s3.secretKey,
                    },
                });
    ...
    private async uploadFile(
        ...
        if (!this.bigbytesConfig.s3?.bucket || this.s3 === undefined) {
        ...
        const upload = new Upload({
            client: this.s3,
            params: {
                Bucket: this.bigbytesConfig.s3.bucket,
                Key: fileId,
                Body: file,
        ...
        try {
            ...
            const url = await getSignedUrl(
                ...
                {
                    expiresIn: this.bigbytesConfig.s3.expirationTime,
                    ...urlOptions,
                },
            );
            return url;
            ...
            Logger.error(
                `Failed to upload file to s3 with endpoint: ${
                    this.bigbytesConfig.s3.endpoint ?? 'no endpoint'
                }. ${error}`,
            );
    ...
    */
    ...
    getExpirationWarning() {
        if (this.isEnabled()) {
            const timeInSeconds =
                this.bigbytesConfig.s3?.expirationTime || 259200;

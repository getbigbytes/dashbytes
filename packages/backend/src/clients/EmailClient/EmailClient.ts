...
type EmailClientArguments = {
    clairdashConfig: Pick<ClairdashConfig, 'smtp' | 'siteUrl' | 'query'>;
};
...
export default class EmailClient {
    ...
    constructor({ clairdashConfig }: EmailClientArguments) {
        this.clairdashConfig = clairdashConfig;

        if (this.clairdashConfig.smtp) {
            Logger.debug(`Create email transporter`);

            const auth: AuthenticationType = this.clairdashConfig.smtp.auth
                .accessToken
                ? {
                      type: 'OAuth2',
                      user: this.clairdashConfig.smtp.auth.user,
                      accessToken: this.clairdashConfig.smtp.auth.accessToken,
                  }
                : {
                      user: this.clairdashConfig.smtp.auth.user,
                      pass: this.clairdashConfig.smtp.auth.pass,
                  };

            this.transporter = nodemailer.createTransport(
                {
                    host: this.clairdashConfig.smtp.host,
                    port: this.clairdashConfig.smtp.port,
                    secure: this.clairdashConfig.smtp.port === 465, // false for any port beside 465, other ports use STARTTLS instead.
                    auth,
                    requireTLS: this.clairdashConfig.smtp.secure,
                    tls: this.clairdashConfig.smtp.allowInvalidCertificate
                        ? { rejectUnauthorized: false }
                        : undefined,
                },
                {
                    from: `"${this.clairdashConfig.smtp.sender.name}" <${this.clairdashConfig.smtp.sender.email}>`,
                },
            );
    ...
    public async sendPasswordRecoveryEmail(link: PasswordResetLink) {
        return this.sendEmail({
            ...
            context: {
                url: link.url,
                host: this.clairdashConfig.siteUrl,
            },
    ...
    public async sendInviteEmail(
        ...
        return this.sendEmail({
            to: invite.email,
            subject: `You've been invited to join Clairdash`,
            template: 'invitation',
            context: {
            ...
            text: `Your teammates at ${userThatInvited.organizationName} are using Clairdash to discover and share data insights. Click on the link below within the next 72 hours to join your team and start exploring your data! ${invite.inviteUrl}?from=email`,
        });
    }
    ...
    public async sendChartCsvNotificationEmail(
        ...
        return this.sendEmail({
            ...
            context: {
                ...
                maxCells: this.clairdashConfig.query.csvCellsLimit,
                host: this.clairdashConfig.siteUrl,
                schedulerUrl,
                expirationDays,
                includeLinks,
    ...
    async sendOneTimePasscodeEmail({
        ...
        const text = `
        Verify your email address by entering the following passcode in Clairdash: ${passcode}
            `;
        return this.sendEmail({
            to: recipient,
            subject,

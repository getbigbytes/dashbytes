...
const configDir = path.join(os.homedir(), '.config', 'bigbytes');
...
export const getConfig = async (): Promise<Config> => {
    ...
    return {
        ...
        context: {
            ...(rawConfig.context || {}),
            apiKey: process.env.BIGBYTES_API_KEY || rawConfig.context?.apiKey,
            project:
                process.env.BIGBYTES_PROJECT || rawConfig.context?.project,
            serverUrl:
                process.env.BIGBYTES_URL || rawConfig.context?.serverUrl,
            proxyAuthorization:
                process.env.BIGBYTES_PROXY_AUTHORIZATION ||

...
const configDir = path.join(os.homedir(), '.config', 'clairdash');
...
export const getConfig = async (): Promise<Config> => {
    ...
    return {
        ...
        context: {
            ...(rawConfig.context || {}),
            apiKey: process.env.CLAIRDASH_API_KEY || rawConfig.context?.apiKey,
            project:
                process.env.CLAIRDASH_PROJECT || rawConfig.context?.project,
            serverUrl:
                process.env.CLAIRDASH_URL || rawConfig.context?.serverUrl,
            proxyAuthorization:
                process.env.CLAIRDASH_PROXY_AUTHORIZATION ||

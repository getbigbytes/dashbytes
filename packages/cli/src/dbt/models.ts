...
import { WarehouseClient, WarehouseTableSchema } from '@bigbytes/warehouses';
...
export const getModelsFromManifest = (
    ...
    if (!isSupportedDbtAdapter(manifest.metadata)) {
        throw new ParseError(
            `dbt adapter not supported. Bigbytes does not support adapter ${manifest.metadata.adapter_type}`,
            {},
        );
    }

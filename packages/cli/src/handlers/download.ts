...
const DOWNLOAD_FOLDER = 'bigbytes';
...
export const downloadHandler = async (
    ...
    if (!projectId) {
        throw new Error(
            'No project selected. Run bigbytes config set-project',
        );
    }
    const chartsAsCode = await bigbytesApi<
...
export const uploadHandler = async (
    ...
    try {
        // Read all files from the bigbytes directory
    ...
    try {
        for (const chart of charts) {
            ...
            const chartData = await bigbytesApi<

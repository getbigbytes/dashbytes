import { OrganizationProject } from '@bigbytes/common';
import inquirer from 'inquirer';
import { URL } from 'url';
import { getConfig, setProject } from '../config';
import GlobalState from '../globalState';
import { bigbytesApi } from './dbt/apiClient';
...
export const setProjectCommand = async (name?: string, uuid?: string) => {
    const projects = await bigbytesApi<OrganizationProject[]>({
        method: 'GET',
    ...
    if (selectedProject !== undefined) {
        ...
        console.error(
            `\n  ✅️ Connected to Bigbytes project: ${projectUrl || ''}\n`,
        );
    } else {

import { OrganizationProject } from '@clairdash/common';
import inquirer from 'inquirer';
import { URL } from 'url';
import { getConfig, setProject } from '../config';
import GlobalState from '../globalState';
import { clairdashApi } from './dbt/apiClient';
...
export const setProjectCommand = async (name?: string, uuid?: string) => {
    const projects = await clairdashApi<OrganizationProject[]>({
        method: 'GET',
    ...
    if (selectedProject !== undefined) {
        ...
        console.error(
            `\n  ✅️ Connected to Clairdash project: ${projectUrl || ''}\n`,
        );
    } else {

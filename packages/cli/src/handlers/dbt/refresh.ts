...
import { checkClairdashVersion, clairdashApi } from './apiClient';

const getProject = async (projectUuid: string) =>
    clairdashApi<Project>({
        method: 'GET',
...
const refreshProject = async (projectUuid: string) =>
    clairdashApi<ApiRefreshResults>({
        method: 'POST',
...
export const refreshHandler = async (options: RefreshHandlerOptions) => {
    ...
    if (project.dbtConnection.type === DbtProjectType.NONE) {
        throw new ParameterError(
            'Clairdash project must be connected to a remote repository. eg: GitHub, Gitlab, etc',
        );
    }

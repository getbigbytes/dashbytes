import { AuthorizationError, formatDate } from '@bigbytes/common';
...
const loginWithPassword = async (url: string) => {
    ...
    const now = new Date();
    const description = `Generated by the Bigbytes CLI on ${formatDate(now)}`;
...
export const login = async (url: string, options: LoginOptions) => {
    ...
    if (url.includes('bigbytes.com')) {
        const cloudServer = url.replace('bigbytes.com', 'bigbytes.cloud');
        console.error(
    ...
    const proxyAuthorization = process.env.BIGBYTES_PROXY_AUTHORIZATION;
    ...
    try {
        if (process.env.CI === 'true') {
            ...
            if (project === undefined) {
                console.error(
                    'Now you can add your first project to bigbytes by doing: ',
                );
                console.error(
                    `\n  ${styles.bold(`⚡️ bigbytes deploy --create`)}\n`,
                );
            }
        }
    } catch {
        ...
        console.error(
            `\n  ${styles.bold(`⚡️ bigbytes config set-project`)}\n`,
        );
    }
};

import { AuthorizationError, formatDate } from '@clairdash/common';
...
const loginWithPassword = async (url: string) => {
    ...
    const now = new Date();
    const description = `Generated by the Clairdash CLI on ${formatDate(now)}`;
...
export const login = async (url: string, options: LoginOptions) => {
    ...
    if (url.includes('clairdash.com')) {
        const cloudServer = url.replace('clairdash.com', 'clairdash.cloud');
        console.error(
    ...
    const proxyAuthorization = process.env.CLAIRDASH_PROXY_AUTHORIZATION;
    ...
    try {
        if (process.env.CI === 'true') {
            ...
            if (project === undefined) {
                console.error(
                    'Now you can add your first project to clairdash by doing: ',
                );
                console.error(
                    `\n  ${styles.bold(`⚡️ clairdash deploy --create`)}\n`,
                );
            }
        }
    } catch {
        ...
        console.error(
            `\n  ${styles.bold(`⚡️ clairdash config set-project`)}\n`,
        );
    }
};

...
import { Project, ProjectType } from '@clairdash/common';
...
const getPreviewProject = async (name: string) => {
    const projects = await clairdashApi<Project[]>({
        method: 'GET',
...
export const previewHandler = async (
    ...
    if (!config.context?.project) {
        console.error(
            styles.warning(
                `\n\nDeveloper preview will be deployed without any copied content!\nPlease set a project to copy content from by running 'clairdash config set-project'.\n`,
            ),
        );
    }

...
import { Project, ProjectType } from '@bigbytes/common';
...
const getPreviewProject = async (name: string) => {
    const projects = await bigbytesApi<Project[]>({
        method: 'GET',
...
export const previewHandler = async (
    ...
    if (!config.context?.project) {
        console.error(
            styles.warning(
                `\n\nDeveloper preview will be deployed without any copied content!\nPlease set a project to copy content from by running 'bigbytes config set-project'.\n`,
            ),
        );
    }

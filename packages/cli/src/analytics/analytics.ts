import { BigbytesUser } from '@bigbytes/common';
import fetch from 'node-fetch';
import { Config, getConfig } from '../config';
import { bigbytesApi } from '../handlers/dbt/apiClient';
...
const identifyUser = async (): Promise<Config['user']> => {
    ...
    if (config.context?.serverUrl && config.context.apiKey) {
        try {
            const user = await bigbytesApi<BigbytesUser>({
                method: 'GET',
                url: '/api/v1/user',
                body: undefined,
            });
...
export class BigbytesAnalytics {
    static async track(payload: Track): Promise<void> {
        try {
            const user = await identifyUser();
            const bigbytesContext = {
                app: {
                    namespace: 'bigbytes',
                    name: 'bigbytes_cli',
                    version: VERSION,
            ...
            const body = {
                ...
                ...payload,
                event: `${bigbytesContext.app.name}.${payload.event}`,
                context: { ...bigbytesContext },
            };
            ...
            await fetch('https://analytics.bigbytes.com/v1/track', {
                method: 'POST',
                headers: {
...
export const analytics: BigbytesAnalytics = new BigbytesAnalytics();

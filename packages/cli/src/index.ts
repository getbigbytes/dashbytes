#!/usr/bin/env node
import { ClairdashError, ValidationTarget } from '@clairdash/common';
...
program
    .version(VERSION)
    .name(styles.title('⚡️clairdash'))
    .description(
        'Developer tools for dbt and Clairdash.\nSee https://docs.clairdash.com for more help and examples',
    )
    .showHelpAfterError(
        styles.bold('Run ⚡️clairdash help [command] for more information'),
    )
    .addHelpText(
        'after',
        `
${styles.bold('Examples:')}
  ${styles.title('⚡')}️clairdash ${styles.bold('generate')} ${styles.secondary(
            '-- generates .yml file for all dbt models',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'generate',
  ...
  ${styles.title('⚡')}️clairdash ${styles.bold('dbt run')} ${styles.secondary(
  ...
  ${styles.title('⚡')}️clairdash ${styles.bold('compile')} ${styles.secondary(
            '-- compiles Clairdash metrics and dimensions',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold('deploy')} ${styles.secondary(
            '-- compiles and deploys Clairdash metrics to active project',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'login https://clairdash.domain.com',
        )} ${styles.secondary('-- logs in to a Clairdash instance')}
`,
    );
...
program
    .command('login <url>')
    .description('Logs in to a Clairdash instance')
    .description(
        'Logs in to a Clairdash instance.\n\n👀 See https://docs.clairdash.com/guides/cli/cli-authentication for more help and examples',
    )
    .addHelpText(
        'after',
        `
${styles.bold('Examples:')}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'login',
        )} https://app.clairdash.cloud ${styles.secondary(
            '-- Logs in to Clairdash Cloud US instance',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'login',
        )} https://eu1.clairdash.cloud ${styles.secondary(
            '-- Logs in to Clairdash Cloud EU instance',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'login',
        )} https://custom.clairdash.domain ${styles.secondary(
            '-- Logs in to a self-hosted instance at a custom domain',
        )}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            'login',
        )} https://custom.clairdash.domain --token 12345 ${styles.secondary(
...
configProgram
    .command('set-project')
    .description(
        'Choose project.\nSee https://docs.clairdash.com/guides/cli/cli-authentication#set-active-project for more help and examples',
    )
    .option('--verbose', undefined, false)
    .addOption(
        new Option(
...
program
    .command('compile')
    .description('Compiles Clairdash resources')
    .option(
        '--project-dir <path>',
        'The directory of the dbt project',
        defaultProjectDir,
...
program
    .command('deploy')
    .description('Compiles and deploys a Clairdash project')
    .option(
        '--project-dir <path>',
        'The directory of the dbt project',
        defaultProjectDir,
...
program
    .command('refresh')
    .description('Refreshes Clairdash project with remote repository')
    .addHelpText(
        'after',
        `
${styles.bold('Examples:')}
  ${styles.title('⚡')}️clairdash ${styles.bold('refresh')}
`,
    )
    .option('--verbose', undefined, false)
...
program
    ...
    .description(
        '[Experimental command] Generates a .yml file for Clairdash exposures',
    )
    .addHelpText(
        'after',
        `
${styles.bold('Examples:')}
  ${styles.title('⚡')}️clairdash ${styles.bold(
            ...
            '-- generates .yml file for all clairdash exposures',
        )}
`,
    )
    .option(
...
const errorHandler = (err: Error) => {
    ...
    if (err.name === 'AuthorizationError') {
        console.error(
            `Looks like you did not authenticate or the personal access token expired.\n\n👀 See https://docs.clairdash.com/guides/cli/cli-authentication for help and examples`,
        );
    } else if (!(err instanceof ClairdashError)) {
        console.error(err);
        if (err.stack) {
        ...
        console.error(
            `  🐛 https://github.com/clairview/clairdash/issues/new?assignees=&labels=🐛+bug&template=bug_report.md&title=${encodeURIComponent(
                err.message,
            )}`,
        );
    }
    ...
    if (nodeVersion.major !== OPTIMIZED_NODE_VERSION) {
        console.warn(
            styles.warning(
                `⚠️ You are using Node.js version ${process.version}. Clairdash CLI is optimized for v${OPTIMIZED_NODE_VERSION} so you might experience issues.`,
            ),
        );
    }
    process.exit(1);

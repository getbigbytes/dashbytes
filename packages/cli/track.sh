...
inform_macos_xcrun_requirement() {
    ...
    if ! command -v xcrun >/dev/null 2>&1; then
        echo "⚠️  macOS users: Before installing the Clairdash CLI, make sure you have 'xcrun' installed."
        echo "    You can install it by running the following command:"
        echo "    xcode-select --install"
        echo "    After installing 'xcrun', you can proceed with the Clairdash CLI installation."
        echo ""
    fi
}

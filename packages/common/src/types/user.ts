...
export type BigbytesUserWithOrg = Required<BigbytesUser>;

export const isUserWithOrg = (
    user: BigbytesUser,
): user is BigbytesUserWithOrg =>
    typeof user.organizationUuid === 'string' &&
    typeof user.organizationName === 'string' &&
    user.organizationCreatedAt instanceof Date &&
    typeof user.role === 'string';

export interface BigbytesUserWithAbilityRules extends BigbytesUser {
    abilityRules: AbilityBuilder<MemberAbility>['rules'];
}

export interface SessionUser extends BigbytesUserWithAbilityRules {
    userId: number;

...
export interface ClairdashUser {
    userUuid: string;
...
export type ClairdashUserWithOrg = Required<ClairdashUser>;

export const isUserWithOrg = (
    user: ClairdashUser,
): user is ClairdashUserWithOrg =>
    typeof user.organizationUuid === 'string' &&
    typeof user.organizationName === 'string' &&
    user.organizationCreatedAt instanceof Date &&
    typeof user.role === 'string';

export interface ClairdashUserWithAbilityRules extends ClairdashUser {
    abilityRules: AbilityBuilder<MemberAbility>['rules'];
}

export interface SessionUser extends ClairdashUserWithAbilityRules {
    userId: number;

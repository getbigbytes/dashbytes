...
// exclude clairdash prefix from variable pattern
export const clairdashVariablePattern =
    /\$\{((?!(clairdash|ld)\.)[a-zA-Z0-9_.]+)\}/g;
...
export const getAllReferences = (raw: string): string[] =>
    (raw.match(clairdashVariablePattern) || []).map(
...
export class ExploreCompiler {
    ...
    compileMetricSql(
        ...
        if (metric.filters !== undefined && metric.filters.length > 0) {
            ...
            const conditions = metric.filters.map((filter) => {
                ...
                // NOTE: date dimensions from explores have their time format uppercased (e.g. order_date_DAY) - see ticket: https://github.com/clairview/clairdash/issues/5998
    ...
    compileDimensionSql(
        ...
        const sql = dimension.sql.replace(clairdashVariablePattern, (_, p1) => {
            if ([currentShortRef, currentRef].includes(p1)) {
                throw new CompileError(
    ...
    compileExploreJoinSql(
        ...
        return join.sqlOn.replace(clairdashVariablePattern, (_, p1) => {
            const compiledReference = this.compileDimensionReference(
                p1,
                tables,
                join.table,

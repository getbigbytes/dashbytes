...
describe('Parse dimension reference', () => {
    ...
    test('should not parse clairdash attribute', () => {
        expect(
            parseAllReferences('${clairdash.attribute.country}', 'table'),
        ).toStrictEqual([]);
    });

    test('should not parse short clairdash attribute', () => {
        expect(parseAllReferences('${ld.attr.country}', 'table')).toStrictEqual(
            [],
        );
    });

    test('should parse references with clairdash prefix', () => {
        expect(
            parseAllReferences('${clairdash_dimension} == 1', 'table'),
        ).toStrictEqual([
            { refName: 'clairdash_dimension', refTable: 'table' },
        ]);
        expect(
            parseAllReferences('${dimension_clairdash} == 1', 'table'),
        ).toStrictEqual([
            { refName: 'dimension_clairdash', refTable: 'table' },
        ]);
        expect(
            parseAllReferences(
                '${clairdash_table.dimension}',
                'clairdash_table',
            ),
        ).toStrictEqual([
            { refName: 'dimension', refTable: 'clairdash_table' },
        ]);
        expect(
            parseAllReferences(
                '${table_clairdash.dimension}',
                'table_clairdash',
            ),
        ).toStrictEqual([
            { refName: 'dimension', refTable: 'table_clairdash' },
        ]);
        expect(

...
describe('Parse dimension reference', () => {
    ...
    test('should not parse bigbytes attribute', () => {
        expect(
            parseAllReferences('${bigbytes.attribute.country}', 'table'),
        ).toStrictEqual([]);
    });

    test('should not parse short bigbytes attribute', () => {
        expect(parseAllReferences('${ld.attr.country}', 'table')).toStrictEqual(
            [],
        );
    });

    test('should parse references with bigbytes prefix', () => {
        expect(
            parseAllReferences('${bigbytes_dimension} == 1', 'table'),
        ).toStrictEqual([
            { refName: 'bigbytes_dimension', refTable: 'table' },
        ]);
        expect(
            parseAllReferences('${dimension_bigbytes} == 1', 'table'),
        ).toStrictEqual([
            { refName: 'dimension_bigbytes', refTable: 'table' },
        ]);
        expect(
            parseAllReferences(
                '${bigbytes_table.dimension}',
                'bigbytes_table',
            ),
        ).toStrictEqual([
            { refName: 'dimension', refTable: 'bigbytes_table' },
        ]);
        expect(
            parseAllReferences(
                '${table_bigbytes.dimension}',
                'table_bigbytes',
            ),
        ).toStrictEqual([
            { refName: 'dimension', refTable: 'table_bigbytes' },
        ]);
        expect(

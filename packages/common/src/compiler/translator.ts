import {
    ...
    isV9MetricRef,
    SupportedDbtAdapter,
    type DbtColumnClairdashDimension,
    type DbtMetric,
    type DbtModelColumn,
    type DbtModelNode,
    type LineageGraph,
...
const convertDbtMetricToClairdashMetric = (
    metric: DbtMetric,
    tableName: string,
    tableLabel: string,
): Metric => {
    ...
    if (metric.calculation_method === 'derived') {
        ...
        try {
            ...
            throw new ParseError(
                `Cannot parse metric '${metric.unique_id}: type ${metric.calculation_method} is not a valid Clairdash metric type`,
            );
        }
...
export const convertTable = (
    ...
    const [dimensions, metrics]: [
        ...
        ([prevDimensions, prevMetrics], column, index) => {
            ...
            const processIntervalDimension = (
                dim: Dimension,
                overrideTimeIntervals: DbtColumnClairdashDimension['time_intervals'],
            ) => {
    ...
    const convertedDbtMetrics = Object.fromEntries(
        dbtMetrics.map((metric) => [
            metric.name,
            convertDbtMetricToClairdashMetric(metric, model.name, tableLabel),
        ]),
    );
                    ...
                    errors: [
                        {
                            ...
                            message:
                                e.message ||
                                `Could not convert dbt model: "${model.name}" in to a Clairdash explore`,
                        },
                    ],
                };

...
import FineGrainedRepo from './assets/fine-grained-repo.png';
import FineGrainedPermissions from './assets/fine-grained-permissions.png';

# Update your project connection

To setup your Clairdash connection you'll need to:

1. Connect to your data warehouse (bigquery, postgres, redshift, snowflake, databricks)
2. Connect to your dbt project

## Open up your Clairdash instance to get started.
...
We always recommend giving read-only permissions to Clairdash, that way you ensure than no data can be manipulated. See each section below for warehouse specific details.

:::info

Since we host your Clairdash for you, you may need to add our static IP addresses to the allow-list for your security group or firewall rules so they don’t block calls from your Clairdash Cloud to your connected databases.

If you login at:

- **app.clairdash.cloud** use **35.245.81.252**
- **eu1.clairdash.cloud** use **34.79.239.130**
...
This is project ID from Google Cloud Platform for the data that you want to connect Clairdash to.
...
For the project you want to connect Clairdash to, just copy its `id` and pop it into the `project` field in the Warehouse Connection form in Clairdash.
...
To connect to BigQuery, you'll need to have a service account that you can use with Clairdash. You can read more about [creating and managing service accounts with Google BigQuery in their docs](https://cloud.google.com/iam/docs/creating-managing-service-accounts).

:::info

If you're creating a new service account, make sure that you save your JSON key file somewhere safe! You'll need it for connecting to Clairdash.

:::

The service account you use with Clairdash will need to have the following roles in your GCP project:
...
Once you have a service account all ready to go, you'll need to add its JSON key file to Clairdash in the `key file` section of the Warehouse Connection page.

#### Timeout in seconds

BigQuery supports query timeouts. By default, the timeout is set to 300 seconds. If a query run by Clairdash takes longer than this timeout to complete, then BigQuery may cancel the query and issue the following error:
...
The priority for the BigQuery jobs that Clairdash executes can be configured with the `priority` configuration in your Warehouse Connection settings. The `priority` field can be set to one of `batch` or `interactive`.
...
The `retries` configuration specifies the number of times Clairdash should retry queries that result in unhandled server errors.

For example, setting `retries` to 5 means that Clairdash will retry BigQuery queries 5 times with a delay. If the query does not succeed after the fifth attempt, then Clairdash will raise an error.

By default, the number of retries is set to 3.

#### Maximum bytes billed

If a value for the `Maximum bytes billed` is set, then queries executed by Clairdash will fail if they exceed the configured maximum bytes threshhold. This configuration should be supplied as an integer number of bytes.
...
This controls what day is the start of the week in Clairdash. `Auto` sets it to whatever the default is for your data warehouse. Or, you can customize it and select the day of the week from the drop-down menu. This will be taken into account when using 'WEEK' time interval in Clairdash.
...
If the database closes its connection while Clairdash is waiting for data, you may see the error `SSL SYSCALL error: EOF detected`. Lowering the [`keepalives_idle` value](https://www.postgresql.org/docs/9.3/libpq-connect.html) may prevent this, because the server will send a ping to keep the connection active more frequently.
...
2. Click the configuration tab for the cluster that you're connecting to Clairdash.
3. Expand the `Advanced options` tab
4. Open the `JDBC/ODBC` tab

<img
  src={DatabricksConnectScreenshot}
  width="922"
  height="608"
  style={{ display: 'block', margin: '0 auto 20px auto' }}
/>
...
<img
  src={DatabricksAccessScreenshot}
  width="1097"
  height="647"
  style={{ display: 'block', margin: '0 auto 20px auto' }}
/>
...
Connecting Clairdash to a hosted dbt project means that you'll be able to keep your Clairdash instance in sync with the changes in your dbt project.

To connect your dbt project, just head to your project connection settings in Clairdash:

<img
  src={ProjectConnectionSettings}
  width="1043"
  height="613"
  style={{ display: 'block', margin: '0 auto 20px auto' }}
/>
...
We don't support connecting Clairdash to [dbt Cloud managed repositories](https://docs.getdbt.com/docs/dbt-cloud/cloud-configuring-dbt-cloud/cloud-using-a-managed-repository). If you're using dbt Cloud and you'd like to use Clairdash, you'll need to have a remote repository for your dbt project in one of the services listed above.
...
Fine-grained access tokens are new special tokens that can only give access to individual repositories on your github account.
You can read more about it on the [Github docs](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token#creating-a-fine-grained-personal-access-token).
...
4. On repository permissions, select `Contents`
...
This should be in the format `my-org/my-repo`. e.g. `clairview/clairdash-analytics`

#### Branch

This is the branch in your GitHub repo that Clairdash should sync to. e.g. `main`, `master` or `dev`
...
- Put `/` if your `dbt_project.yml` file is in the main folder of your repo (e.g. clairview/clairdash-analytics/dbt_project.yml)
- Include the path to the sub-folder where your dbt project is if your dbt project is in a sub-folder in your repo. For example, if my project was in clairview/clairdash-analytics/dbt/dbt_project.yml, I'd write `/dbt` in this field.
...
It's the dataset/schema in your data warehouse that Clairdash will look for your dbt models. By default, we set this to be the same value as you have as the default in your `profiles.yml` file.
...
If you've used [environment variables in your dbt `profiles.yml` file](https://docs.getdbt.com/reference/dbt-jinja-functions/env_var), you can add these to Clairdash here.
...
So, in Clairdash, I'd add a new environment variable and put `key` as `DBT_USER` and `value` as `myspecialuserkey123`.
...
You can find this in the GitLab URL when you're in your repo. This should be in the format `my-org/my-repo`. e.g. if my browser had `https://gitlab.com/clairview/clairdash-analytics.gitlab.io`, I'd put in: `clairview/clairdash-analytics` as my repository in Clairdash.

#### Branch

This is the branch in your GitLab repo that Clairdash should sync to. e.g. `main`, `master` or `dev`
...
If your `dbt_project.yml` file is in the main folder of your repo (e.g. `clairview/clairdash-analytics/dbt_project.yml`),
then you don't need to change anything in here. You can just leave the default value we've put in.

If your dbt project is in a sub-folder in your repo (e.g. `clairview/clairdash-analytics/dbt/dbt_project.yml`), then
...
This is the branch in your repository that Clairdash should sync to. e.g. `main`, `master` or `dev`
...
Unsuitable for production and only available for Clairdash instances installed on your local machine

:::

To start Clairdash with the option to connect to a local dbt project, you must specify the directory of the dbt project when
...
This is the branch in your Bitbucket repo that Clairdash should sync to. e.g. `main`, `master` or `dev`

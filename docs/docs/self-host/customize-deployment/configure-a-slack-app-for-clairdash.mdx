...
# Configure a slack integration for self-hosted Clairdash

In this guide we will show you how you can enable [the Slack integration](/references/slack-integration.mdx) on your on self-hosted Clairdash server.
...
Then copy this manifest to allow URL unfurls in your app. **Make sure you update `your Clairdash URL` in the manifest below** (for example, `app.clairdash.cloud` or `eu1.clairdash.cloud`).

```yaml
display_information:
  name: Clairdash
  description: Share Clairdash URLs on your Slack
  background_color: '#7262ff'
features:
  bot_user:
    display_name: Clairdash
    always_online: false
  unfurl_domains:
    - your-clairdash-deployment-url.com
oauth_config:
  redirect_urls:
    - https://your-clairdash-deployment-url.com/api/v1/slack/oauth_redirect
  scopes:
    bot:
      - app_mentions:read
      - channels:join
...
settings:
  event_subscriptions:
    request_url: https://your-clairdash-deployment-url.com/slack/events
    bot_events:
      - app_mention
      - link_shared
  interactivity:
...
Now you need to add the following environment variables to your Clairdash server using the credentials we previously copied
...
Restart your Clairdash service, now you should be able to use [the Slack integration](/references/slack-integration.mdx) on your self-hosted Clairdash.

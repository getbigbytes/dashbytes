...
# Configure Clairdash to use an external database

> Note: Clairdash requires a **PostgreSQL** database (version 12 or greater).
...
The following configuration shows an example of how to configure Clairdash to use an external database. You can use this as a starting point for your own configuration.
...
# Configure Clairdash to use an external database
externalDatabase:
  host: clairdash-db.mycompany.com
  port: 5432
  user: clairdash
  password: clairdash
  database: clairdash
...
Clairdash requires the following extensions to be installed on the database:

- `uuid-ossp` - used for generating unique IDs

## Migrations

Migrations are ran automatically on starting the Clairdash server or workers. When upgrading Clairdash, migrations

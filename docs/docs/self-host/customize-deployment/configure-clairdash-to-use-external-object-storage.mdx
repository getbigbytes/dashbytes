...
# Configure Clairdash to use external storage

On Clairdash, we generate some files like:

- Images for Slack unfurl
- Images on scheduled deliveries
- Results on CSV format.

These files can be stored locally and can be served through Clairdash,
however, if you run Clairdash on multiple containers, these files might not be available when you requested,
...
To solve this, we suggest you enable Cloud storage, so Clairdash can store the files in a cloud (GCP buckets, S3 storage...)
...
- give it an unique `Bucket name` like - `clairdash-cloud-file-storage-eu`
...
## Configure Clairdash to use S3 credentials

Now you have the bucket and the S3 credentials for your cloud storage,
you can provide the following ENV variables to Clairdash so it can use the cloud storage using S3 library

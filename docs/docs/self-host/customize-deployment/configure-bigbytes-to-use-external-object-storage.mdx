...
# Configure Bigbytes to use external storage

On Bigbytes, we generate some files like:

- Images for Slack unfurl
- Images on scheduled deliveries
- Results on CSV format.

These files can be stored locally and can be served through Bigbytes,
however, if you run Bigbytes on multiple containers, these files might not be available when you requested,
...
To solve this, we suggest you enable Cloud storage, so Bigbytes can store the files in a cloud (GCP buckets, S3 storage...)
...
- give it an unique `Bucket name` like - `bigbytes-cloud-file-storage-eu`
...
## Configure Bigbytes to use S3 credentials

Now you have the bucket and the S3 credentials for your cloud storage,
you can provide the following ENV variables to Bigbytes so it can use the cloud storage using S3 library

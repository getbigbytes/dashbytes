...
# Configure Clairdash to use passwords or SSO for authentication

## Passwords

We recommend [adding the SMTP environment variables](/references/environmentVariables#smtp-environment-variables) so Clairdash can display a `Forgot your password?` button in the login page and send emails to reset passwords.

You can override a user password in just a few steps:

1. Open the bash terminal for the docker Clairdash container
...
Clairdash supports Okta as an authentication provider. The integration uses OpenID Connect (OIDC) to authenticate
users and JIT provisioning to create users in Clairdash when they first log in.
...
On the following page you'll need to use the following settings, replace `{{ clairdash_url }}` with the URL of your
Clairdash instance. For example if you normally access Clairdash at `https://clairdash.example.com/login` then you
should use `https://clairdash.example.com` as your `{{ clairdash_url }}`.

- **Grant type**: Authorization Code
- **Sign-in redirect URIs**: `{{ clairdash_url }}/api/v1/oauth/redirect/okta`
- **Sign-out redirect URIs**: `{{ clairdash_url }}`
...
- **Initiate login URI**: `{{ clairdash_url }}/api/v1/login/okta`
...
If you want to use groups to control access to Clairdash, you'll need to configure Okta and Clairdash to support this.
...
### Configuring Clairdash for Okta

You'll need to set the following environment variables in your Clairdash deployment:
...
### Enable Automatic Assignment of Okta Users to Groups in Clairdash

:::warning Deprecated feature

This feature is deprecated and will be removed in a future release.
For more information on how to provision users and groups in Clairdash, see the [SCIM integration](/references/scim-integration.mdx) documentation.

:::

Okta users will automatically be assigned to the same groups in Clairdash as they are in Okta if you have [configured Okta to share groups with Clairdash](../../references/groups.mdx#using-okta-to-manage-groups-in-clairdash). To enable this functionality, ensure the following environment variable is set:

| Variable                 | Description                               | Required? |
| ------------------------ | ----------------------------------------- | --------- |
| `AUTH_ENABLE_GROUP_SYNC` | If `"true"` enables group sync from Okta. |           |

read more about [Using OKTA to manage groups in Clairdash](/references/groups/#using-okta-to-manage-groups-in-clairdash)
...
- Authorized JavaScript Origins: `https://{{ clairdash_domain }}`
- Authorized redirect URIs: `https://{{ clairdash_domain }}/api/v1/oauth/redirect/google`

Where `{{ clairdash_domain }}` is the domain you use to sign in to Clairdash such as `mycompany.clairdash.com`

These environment variables must be provided to Clairdash to enable you to control Single Sign On (SSO) functionality for Google
...
- **Type**: Web
- **URI**: `{{ clairdash_url }}/api/v1/oauth/redirect/azuread`
...
### Configuring Clairdash for Azure AD
...
Clairdash supports OpenID Connect-compliant SSO providers, via our configurable OIDC connector.

### Configuring Clairdash for OpenID Connect

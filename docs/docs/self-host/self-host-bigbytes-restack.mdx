...
4. [Deploy Bigbytes on Restack](#deploy-bigbytes-on-restack).
5. [Start using Bigbytes](#start-using-bigbytes).
6. [Deploy multiple instances of Bigbytes](#deploy-multiple-instances-of-bigbytes).
...
To deploy Bigbytes in your own AWS infrastructure with Restack, you will need to add your credentials as the next step.
...
The cluster creation process will start automatically. Once the cluster is ready, you will get an email on the email id connected with your account. <br/>Creating a cluster is a one-time process. From here you can add other open source tools or multiple instances of Bigbytes in the same cluster.

![creating-cluster](./assets/restack-creating-cluster.png)
![cluster-created](./assets/restack-cluster-created.png)

Any application you deploy in your cluster will be accessible via a free **restack domain**.<br/>Contact the Restack team via chat to set a custom domain for your Bigbytes instances.

### Deploy Bigbytes on Restack

1. Click _Add application_ from the Cluster description or go to the Applications tab in the left hand side navigation.
2. Click _Bigbytes_.
   ![select-bigbytes](./assets/select-bigbytes.png)
3. Select the cluster you have already provisioned.
4. Click _Add application_.

### Start using Bigbytes

Bigbytes will be deployed on your cluster and you can access it using the link under the _URL_ tab.
![bigbytes-deployed](./assets/bigbytes-deployed.png)
You can also check the workloads and volumes that are deployed within Bigbytes.
![access-bigbytes](./assets/bigbytes-access.png)

### Deploy multiple instances of Bigbytes

Restack makes it easier to deploy multiple instances of Bigbytes on the same or multiple clusters. <br/>So you can test the latest version before upgrading or have a dedicated instance for development and for production.

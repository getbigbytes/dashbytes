...
import ScimClairdashGenerateToken from './assets/scim/scim-clairdash-generate-token.png';
import ScimClairdashGeneratedToken from './assets/scim/scim-clairdash-generated-token.png';
import ScimClairdashTokensList from './assets/scim/scim-clairdash-tokens-list.png';
import ScimClairdashUserSettings from './assets/scim/scim-clairdash-user-settings.png';
import ScimClairdashViewURL from './assets/scim/scim-clairdash-view-url.png';
...
SCIM Integration is only available on Clairdash Enterprise plans. For more information on our plans, visit our [pricing page](https://www.clairdash.com/pricing).
...
## SCIM Setup within Clairdash

1. Sign into your Clairdash instance, click your initials at the top-right, and select **User Settings**.

   <img
     src={ScimClairdashUserSettings}
     width="483"
     height="278"
...
3. Click **Generate new token**.
   ...
   <img
     src={ScimClairdashGenerateToken}
     width="754"
     height="375"
     style={{ display: 'block', margin: '0 auto 20px auto' }}
   />
...
4. Once generated, copy and save it in a safe place, as it cannot be viewed again once the modal is closed.
   ...
   <img
     src={ScimClairdashGeneratedToken}
     width="670"
     height="266"
     style={{ display: 'block', margin: '0 auto 20px auto' }}
   />
...
### Step 2 - Connect SCIM to Clairdash from Okta
...
5. Fill in the following fields:
   ...
   - **API Token**: `Bearer YOUR_SCIM_TOKEN` (See SCIM Setup within Clairdash above for generating a token)
...
7. More options should be available. In **Provisioning > To App**, select **Edit**.
   ...
   - Click **Save**.
8. Test the integration by clicking **Assignments**. Select **Assign > Assign to people**. Choose a user and click **Assign > Save and Go Back**. This user should be created in your Clairdash instance.
...
Clairdash will sync the active status from Okta to Clairdash. For example, if a user is provisioned as inactive or is deactivated in Okta, that user will still exist in Clairdash marked as inactive, meaning they will be unable to use the platform.
...
### Step 1 - Connect SCIM to Clairdash from Azure
...
### Step 2 - Connect SCIM to Clairdash from Azure
...
Clairdash will sync the active status from Azure to Clairdash. For example, if a user is provisioned as inactive or is deactivated in Azure, that user will still exist in Clairdash marked as inactive, meaning they will be unable to use the platform.
...
```
curl --location --request PATCH 'https://my.clairdash.com/api/v1/scim/organization-access-tokens/<personal-access-token-uuid>/rotate' \

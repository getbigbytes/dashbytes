...
You can run queries against the tables in your Clairdash project.
...
For example, if we had this in our schema.yml files in dbt, we'd see a Table called `users` in Clairdash.
...
You can read more about [adding Tables to Clairdash here](/guides/adding-tables-to-clairdash).
...
| Property                                          | Value                                 | Note                                                                                                                                                 |
| ------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| label                                             | string                                | Custom label. This is what you'll see in Clairdash instead of the Table name.                                                                        |
...
### If you've added a new dbt model to your project, you need to do `dbt run` + `dbt refresh` before it will appear in Clairdash

Clairdash gets information about your data models from dbt.
...
to the table it generates, then you need to do two things before your changes will appear in Clairdash:
...
2. **Click `dbt refresh` in Clairdash.**
   This will re-sync your dbt project in Clairdash so that any changes you made
   to your dbt models is shown in Clairdash (e.g. adding a new table).
...
`sql_filter` adds a filter to the table that cannot be removed in Clairdash. It is automatically added to the compiled SQL when running queries.
...
Any queries that I run using the `Sales` table in Clairdash will always have a filter for `sales_region = 'EMEA'` in their compiled SQL

```sql
SELECT
  [...]
FROM `clairdash`.`prod`.`sales`
...
Using `sql_filter` with [user attributes](/references/user-attributes) allows you to set up [row-level security in your tables](/references/user-attributes#row-filtering-with-sql_filter). You can reference user attributes in your `sql_filter` using `${clairdash.attributes.my_attribute_name}`
...
Clairdash can use [user attributes](./user-attributes.mdx) to limit some tables to some users.

...
You can run queries against the tables in your Bigbytes project.
...
Bigbytes gets information about your data models from dbt.
...
to the table it generates, then you need to do two things before your changes will appear in Bigbytes:
...
2. **Click `dbt refresh` in Bigbytes.**
   This will re-sync your dbt project in Bigbytes so that any changes you made
   to your dbt models is shown in Bigbytes (e.g. adding a new table).
...
Using `sql_filter` with [user attributes](/references/user-attributes) allows you to set up [row-level security in your tables](/references/user-attributes#row-filtering-with-sql_filter). You can reference user attributes in your `sql_filter` using `${bigbytes.attributes.my_attribute_name}`
...
Bigbytes can use [user attributes](./user-attributes.mdx) to limit some tables to some users.

...
You can embed Clairdash content in your website or application securely using Clairdash embedding.

Embedded Clairdash content is available to view by anyone (not just folks with a Clairdash login).

So, for example, you could embed a dashboard in your product, and anyone who has access to your product would have access to that dashboard. No need to login to Clairdash.

We make sure that the links are secure and have a set expiry time that you pick (more on that below).

:::info

Embedding is a beta feature available to all Clairdash Cloud users, [get in touch](https://clairdash.typeform.com/to/BujU5wg5) to have this feature enabled in your account.

:::

For more of a deep dive into setting up and using embedding in Clairdash, check out our [How to embed content guide](../guides/how-to-embed-content.mdx).

## Known limitations

Since embedding is a beta feature it's still frequently being worked on by the Clairdash team. As of right now there are a few known limitations:
...
Although you can generate URLs directly from Clairdash with a long expiration using [`generate and copy URL`](#generate--copy-url), it is recommended to generate your own JWT embed tokens in your backend (using the [`code snippet`](#code-snippet)) with a short expiration using your `secret` to make sure people can't be using embed URLs outside your app.
...
The filters shown in the embedded dashboard will act like they do in Clairdash. Users interacting with the embedded dashboard will be able to change the value and operator of your filters. They cannot add new filters, remove existing filters, change the field used in the filter, or change the tiles the filter is applied to.
...
Although you can generate URLs directly from Clairdash with a long expiration, it is recommended to generate your own JWT embed tokens in your backend with a short expiration using your `secret` to make sure people can't be using embed URLs outside your app.
...
You can pass user metadata (specifically, an external user ID) from your application so that anytime someone views your embedded dashboard and runs queries in Clairdash, these query logs are enriched with this user metadata. Specifcally, this user metadata gets added to the [query tags](/references/usage-analytics.mdx#query-tags) for queries run in Clairdash.

To assign an external ID, you can update the embed code snippet like this:

```node
import jwt from 'jsonwebtoken';
const CLAIRDASH_EMBED_SECRET = 'secret'; // replace with your secret
...
const token = jwt.sign(data, CLAIRDASH_EMBED_SECRET, { expiresIn: '1 hour' });
const url = `https://analytics.clairdash.cloud/embed/${projectUuid}/#${token}`;
```

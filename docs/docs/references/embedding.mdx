...
You can embed Bigbytes content in your website or application securely using Bigbytes embedding.
...
For more of a deep dive into setting up and using embedding in Bigbytes, check out our [How to embed content guide](../guides/how-to-embed-content.mdx).
...
You can pass user metadata (specifically, an external user ID) from your application so that anytime someone views your embedded dashboard and runs queries in Bigbytes, these query logs are enriched with this user metadata. Specifcally, this user metadata gets added to the [query tags](/references/usage-analytics.mdx#query-tags) for queries run in Bigbytes.

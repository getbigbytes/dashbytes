...
User attributes provide a way to customise Bigbytes charts, dashboards, and other behaviour depending on the user
...
models:
  - name: example
    meta:
      sql_filter: ${bigbytes.user.email} = 'example@digi-trans.org'
```
...
based on the user's attributes. To reference a user attribute in your sql, use the special bigbytes reference
`${bigbytes.attributes.<attribute_name> }`.
...
2. **Multiple attribute values joined with `OR`.** In the example below, users with `team_name = 'HR' OR team_name = 'C-Suite'` have access to the `salary` dimension in Bigbytes.
...
returned from the joined table. To reference a user attribute in your sql, use the special bigbytes reference
`${ bigbytes.attributes.<attribute_name> }`. For example, if you have a user attribute called `sales_region` you can
use it in your sql like this:
...
In this example, the `${bigbytes.attributes.fruit}` will be replaced with `'kiwi','orange','coconut'`.

...
User attributes provide a way to customise Clairdash charts, dashboards, and other behaviour depending on the user
...
## Using user attributes in Clairdash

There are several places in Clairdash where you can customise behaviour based on user attributes.

When referencing user attributes in SQL you can use the following [SQL variables](./sql-variables.mdx):

- `${clairdash.attributes.my_attr_1}` - a user attribute called `my_attr_1`
  - (optional) `ld` as an alias for `clairdash`
  - (optional) `attribute` or `attr` as an alias for `attributes`
- `${clairdash.user.<intrinsic_attribute>}` - reference an `intrinsic_attribute` of the current Clairdash user
  - (optional) `ld` as an alias for `clairdash`
...
models:
  - name: example
    meta:
      sql_filter: ${clairdash.user.email} = 'example@digi-trans.org'
```

If you are self hosting you can enable [SMTP](/self-host/customize-deployment/configure-smtp-for-clairdash-email-notifications) or [SSO](/self-host/customize-deployment/use-sso-login-for-self-hosted-clairdash) authentication to allow users to verify their email address.
...
based on the user's attributes. To reference a user attribute in your sql, use the special clairdash reference
`${clairdash.attributes.<attribute_name> }`.
...
models:
  - name: my_model
    meta:
      sql_filter: ${TABLE}.sales_region IN (${clairdash.attributes.sales_region})
```
...
1. **Multiple attributes joined with `AND`.** In the example below, only users with `is_admin: "true" AND team_name: "HR"` have access to the `salary` dimension in Clairdash.
...
2. **Multiple attribute values joined with `OR`.** In the example below, users with `team_name = 'HR' OR team_name = 'C-Suite'` have access to the `salary` dimension in Clairdash.
...
returned from the joined table. To reference a user attribute in your sql, use the special clairdash reference
`${ clairdash.attributes.<attribute_name> }`. For example, if you have a user attribute called `sales_region` you can
use it in your sql like this:
    ...
    meta:
      joins:
        - join: joined
          sql_on: >
            ${base}.id = ${joined}.id
            AND ${joined}.sales_region = ${clairdash.attributes.sales_region}
```
...
models:
  - name: my_model
    meta:
      sql_filter: ${TABLE}.fruit IN (${clairdash.attributes.fruit})
```

In this example, the `${clairdash.attributes.fruit}` will be replaced with `'kiwi','orange','coconut'`.

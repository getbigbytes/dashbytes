...
In Clairdash, metrics are used to summarize dimensions or, sometimes, other metrics.
...
To add a metric to Clairdash using the `meta` tag, you define it in your dbt project under the dimension name you're trying to describe/summarize.
...
| --------------------------------------------------- | -------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| label                                               | No       | string                         | Custom label. This is what you'll see in Clairdash instead of the metric name.                                                                                                                                                                       |
| [type](#metric-types)                               | Yes      | metric type                    | Metrics must be one of the supported types.                                                                                                                                                                                                          |
| [description](#adding-your-own-metric-descriptions) | No       | string                         | Description of the metric that appears in Clairdash. A default description is created by Clairdash if this isn't included                                                                                                                            |
| [sql](#using-custom-sql-in-aggregate-metrics)       | No       | string                         | Custom SQL used to define the metric.                                                                                                                                                                                                                |
| hidden                                              | No       | boolean                        | If set to `true`, the metric is hidden from Clairdash. By default, this is set to `false` if you don't include this property.                                                                                                                        |
...
Filters are applied to metrics any time that metric is used in Clairdash. Filters can only be used with [aggregate metric types](#metric-categories).
...
To use special characters such as `%!_>` in your filter value you can either put the value in quotes, or escape special characters with a backslash `\`. For example,
if you wanted to filter for subscription status of `is_subscribed` you can write the metric in one of these ways:
...
You can use the `format` parameter to have your metrics show in a particular format in Clairdash. These are the options:

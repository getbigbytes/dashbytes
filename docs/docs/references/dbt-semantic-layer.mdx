...
You can use Clairdash to explore the semantic models you've defined in [dbt's Semantic Layer](https://www.getdbt.com/product/semantic-layer).

Note that this integration is **_separate_** to the Clairdash Semantic Layer (which is configured in your dbt project under the `meta` tags in your .yml files).

:::info

The dbt Semantic Layer integration is only available to [Clairdash Cloud users](https://www.clairdash.com/pricing). To have the dbt Semantic Layer integration activated on your account, please message support@digi-trans.org.

:::

## Connect Clairdash to your dbt Semantic Layer

Before you can connect your dbt Semantic Layer to Clairdash, you need to:
...
Copy-paste the token into `Service Token` in the Clairdash Semantic Layer Integration page.
...
Once you have your dbt Semantic Layer connected to a project in Clairdash, you can explore your Semantic Layer connection by clicking `new` --> `query using dbt Semantic Layer`.
...
Dashboards can only contain charts from one semantic layer connection. That means charts created using the Clairdash Semantic Layer cannot be used on dashboards that contain charts created from the dbt Semantic Layer.

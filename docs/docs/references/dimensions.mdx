...
For a dimension to appear in Clairdash, you just need to declare it in your dbt model's YAML file.
...
models:
  - name: my_model
    columns:
      - name: user_id # will be "User id" in ClairDash
    ...
    columns:
      - name: revenue_gbp_total_est
        ...
        meta:
          dimension:
            type: string
            label: 'Total revenue' # this is the label you'll see in Clairdash
            description: 'My custom description' # you can override the description you'll see in Clairdash here
...
| ------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| label                                       | No       | string                                                                                                                                            | Custom label. If you set this property, this is what you'll see in Clairdash instead of the dimension name.                                                                                                                                                |
| [type](#type)                               | No       | Dimension type                                                                                                                                    | The dimension type is automatically pulled from your table schemas in Clairdash but you can override the type using this property.                                                                                                                         |
| description                                 | No       | string                                                                                                                                            | Description of the dimension in Clairdash. You can use this to override the description you have for the dimension in dbt.                                                                                                                                 |
...
| hidden                                      | No       | boolean                                                                                                                                           | If set to `true`, the dimension is hidden from Clairdash. By default, this is set to `false` if you don't include this property.                                                                                                                           |
...
The types of your dimensions are pulled from your data warehouse, automatically. You can override these types using the `type` meta tag in your .yml file. If you run `clairdash generate` to generate your .yml files, then Clairdash will add the `type` from your data warehouse to your .yml files automatically.
...
Column descriptions in your YAML file are automatically pulled into Clairdash and you can spot them if you hover over the dimension name 👀
...
When you surround text with double or single quotes it will escape the text between so that any special characters recognized by YAML will still pass through to the Clairdash UI.
...
When you use `>-` it allows you to type descriptions that are multiple lines long in the YAML file, but the text will be combined into a single line when parsed. The `clairdash generate` command will automatically add this to keep YAML files easy to read.
...
- name: product_tier
  description: >-
    ...
    and it will be combined in the Clairdash UI
```

Will appear like this in the Clairdash UI:

```
This is a longer description......that requires multiple lines and it will be combined in the Clairdash UI
```

### Vertical bar for preserving line breaks

If you need line breaks to stay in place when they show up in the Clairdash UI, you can use a `|` character like this:
...
And in Clairdash UI it will appear like this:
...
Clairdash automatically adds intervals for dimensions that are timestamps or dates, so you don't have to!
...
Clairdash breaks this out into the default intervals automatically. So, this is how `created` appears in our Clairdash project:
...
In this example, `created` would now appear as a single, timestamp dimension without a drop-down list of time intervals in Clairdash:
...
If you specify time intervals manually, then this overrides the default time intervals used by Clairdash.
...
You can use the `format` parameter to have your dimensions show in a particular format in Clairdash.
...
Clairdash users can interact with dimension values by clicking on them. By adding custom `urls` you can configure
...
The `${ value.formatted }` will be replaced with the value of the company name in the Clairdash UI at query run time.
The `${ row.company.company_id.raw }` will be replaced with the value of the company id in the Clairdash UI at query run time.
...
| Tag                                         | Description                                                                                  |
| ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `${ value.formatted }`                      | The exact value of the dimension as seen in the Clairdash UI. For example "$1,427.20"        |
| `${ value.raw }`                            | The raw value of the dimension returned from the underlying SQL query. For example "1427.2 " |
| `${ row.table_name.column_name.formatted }` | The exact value of the column as seen in the Clairdash UI. For example "$1,427.20"           |
...
Clairdash can use `user attributes` to limit some dimensions to some users.
...
Clairdash dimensions and custom metrics are protected by this feature, however,
...
An additional dimension is not included in your dbt .sql file, so it's not written to your data warehouse. When used in Clairdash, it just adds the dimension definition to your SQL query (so it's "created" at runtime).

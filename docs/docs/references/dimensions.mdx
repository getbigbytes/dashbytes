...
Column descriptions in your YAML file are automatically pulled into Bigbytes and you can spot them if you hover over the dimension name 👀
...
Bigbytes users can interact with dimension values by clicking on them. By adding custom `urls` you can configure
...
The `${ value.formatted }` will be replaced with the value of the company name in the Bigbytes UI at query run time.
The `${ row.company.company_id.raw }` will be replaced with the value of the company id in the Bigbytes UI at query run time.
...
| `${ row.table_name.column_name.formatted }` | The exact value of the column as seen in the Bigbytes UI. For example "$1,427.20"           |
...
Bigbytes dimensions and custom metrics are protected by this feature, however,
...
An additional dimension is not included in your dbt .sql file, so it's not written to your data warehouse. When used in Bigbytes, it just adds the dimension definition to your SQL query (so it's "created" at runtime).

...
Joins let you connect different models to each other so that you can explore more than one model at the same time in Clairdash and see how different parts of your data relate to each other.
...
When you open Clairdash, the dimensions and metrics from your joined model will appear below the ones in your selected model.
...
Sometimes, you might want to use a model as a join, but you don't want it to appear as a table to explore on its own in the list of Tables. You can use labels in your dbt project to help manage these tables and the `table configuration` settings in your Clairdash project.

#### Step 1: Add tags to your models

Models that are used in joins still need to be compiled in Clairdash. So, we suggest using some labels to tag the models that need to compiled in Clairdash with `clairdash`. Then, tag the models that you want users to explore with `clairdash-explore`.

For example, `map_users_organizations` in our example below is a table we want to use as a join, but we don't want it to appear as a table on its own that can be explored. So, in the `dbt_project.yml` file, we'll add the `tag: clairdash`, but we won't add the tag `clairdash-explore`
...
When deploying clairdash via the CLI or using GitHub actions, you should use the dbt selector `-s clairdash` to make sure you don't compile unnecessary models.

#### Step 2: Update your Tables Configuration in the Clairdash app

In the Clairdash app, under the `project settings` --> `tables configuration`, you can filter by `clairdash-explore` so all the intermediary joins are hidden and users have a small and curated list of models then can explore from.

...
Sometimes the format of things in your dbt project is different to how you want it to look in Clairdash. That's okay! We've built a bunch of features to help you to format the fields in your dbt project so that the data in your Clairdash project looks exactly like you want it to 🥸
...
Sometimes, we have a bunch of columns in our YAML files that we might not want to include in Clairdash. For example, columns with PII data, or the same date data, but at different levels of date granularity.

It's easy to hide columns from Clairdash. All you need to do is add two words to your column: `hidden: true`.
...
By default, Clairdash pulls in the descriptions you've included for your dimensions. But, you can override the description you see in Clairdash using the `description` property.
...
models:
  - name: users
    columns:
      - name: user_id
        description: "Id generated by the Clairdash API on user's first login. On legacy systems, SHA64. On new systems since 2012, FARM_FINGERPRINT()"
        meta:
          dimension:
            description: 'Unique identifier for a user'
```

You can see the descriptions of your dimensions when you hover over the fields in Clairdash.
...
If you don't add a custom description for your metric, Clairdash will show a description for you in the app, by default. To override this default description, you can use the `description` property.
...
Check out [this doc](https://docs.clairdash.com/references/metrics#metric-configuration) to see all of the other properties you can customize for metrics.

### Changing the name of your fields in Clairdash

Sometimes, the labels we use for the fields in our dbt project aren't very user friendly. We might want to change these in Clairdash, and we can!

To change the name you'll see for your field in Clairdash, you just use the `label` property.
...
Check out [this doc](https://docs.clairdash.com/references/dimensions#dimension-configuration) to see all of the other properties you can customize for dimensions, and [this one](https://docs.clairdash.com/references/metrics#metric-configuration) for all of the other properties you can customize for your metrics.
...
| Original number | Round value | How it will appear in Clairdash |
| --------------- | ----------- | ------------------------------- |
| 121.854         | 2           | 121.85                          |
| 121.854         | 1           | 121.9                           |
| 121.854         | 0           | 123                             |
...
Check out [this doc](https://docs.clairdash.com/references/dimensions#dimension-configuration) to see all of the other properties you can customize for dimensions.
...
Check out [this doc](https://docs.clairdash.com/references/dimensions#compact-values) to see all the other compact values.
...
Check out [this doc](https://docs.clairdash.com/references/metrics#compact-values) to see all the other compact values.
...
In Clairdash, you can use the `format` label to add units to your fields.

Here's an example of how different formats will affect your values:

| Original value | Format value | How it will appear in Clairdash |
| -------------- | ------------ | ------------------------------- |
| 121.854        | 'gbp'        | £121.854                        |
| 121.854        | 'usd'        | $121.9                          |

You can see which format types are currently available for [dimensions](https://docs.clairdash.com/references/dimensions#:~:text=number%20of%20digits-,format,-No), and for [metrics.](https://docs.clairdash.com/references/metrics#:~:text=number%20of%20digits-,format,-No)
...
To see which format types are available for dimensions, [check the reference docs here.](https://docs.clairdash.com/references/dimensions#:~:text=number%20of%20digits-,format,-No). Check out [this doc](https://docs.clairdash.com/references/dimensions#dimension-configuration) to see all of the other properties you can customize for dimensions.
...
To see which format types are available for metrics, [check the reference docs here.](/references/metrics#:~:text=number%20of%20digits-,format,-No)
...
## Formatting your fields in the Clairdash UI

You can also format your fields in the Clairdash UI. This is useful if you want to format your fields quickly without having to change your YAML files.

Currently you can format 2 types of fields in the Clairdash UI:

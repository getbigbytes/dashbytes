...
Clairdash allows you to embed your dashboards using expirable URLs and tokens (embed URLs from now on). This is a great way to enable self-serve analytics in your own application and platform by leveraging the insights you've got in Clairdash and making them available to your own users.

:::info

Embedding is available to all Clairdash Cloud users, [get in touch](https://clairdash.typeform.com/to/BujU5wg5) to have this feature enabled in your account.
...
Although you can generate URLs directly from Clairdash with a long expiration
...
## Do I need a Clairdash account to be able to view embedded content?

No, embedded Clairdash content is available to view by anyone (not just folks with a Clairdash login).
...
### Create a user attribute in Clairdash

First, we need to create a [user attribute](../references/user-attributes.mdx) in Clairdash that we can use to assign our users to a specific shop. I create an attribute called `shop_id` and I set the default value to `all`.
...
models:
  - name: orders
    meta:
      sql_filter: ${clairdash.attributes.shop_id} = 'all' OR ${TABLE}.shop_id = ${clairdash.attributes.shop_id}
```
...
You can adjust the value of the user attribute based on a variable coming from your app so, for example, the user attribute for `shop_id` in the Clairdash embedded dashboard would change depending on which `shop_id` the user belonged to in my app.

To do this, I would pass Clairdash an external value for `shop_id` in the embed code snippet. This could look something like:

...
Bigbytes allows you to embed your dashboards using expirable URLs and tokens (embed URLs from now on). This is a great way to enable self-serve analytics in your own application and platform by leveraging the insights you've got in Bigbytes and making them available to your own users.
...
Although you can generate URLs directly from Bigbytes with a long expiration
...
## Do I need a Bigbytes account to be able to view embedded content?
...
### Create a user attribute in Bigbytes

First, we need to create a [user attribute](../references/user-attributes.mdx) in Bigbytes that we can use to assign our users to a specific shop. I create an attribute called `shop_id` and I set the default value to `all`.

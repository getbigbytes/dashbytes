...
You can check to see if your changes break any of the content in your project using `bigbytes validate`. This command will run the validator which will return a list of content in your project that is broken and details about the errors.
...
### Validate your changes against your project by running bigbytes validate

You can run `bigbytes validate` to check if your changes break any of the content in production.
By default, `bigbytes validate` will check your changes against the content in the project you've selected on the CLI.
You can change your project using `bigbytes config set-project`.
...
These errors will not be reflected on the validation table on Bigbytes settings.
...
`bigbytes validate` will return an error (return code 1) if there is at least 1 validation error on your project.
...
To learn more about setting up GitHub actions for Bigbytes, check out the docs [here](/snippets/automatically-deploy-your-changes-using-github-action)

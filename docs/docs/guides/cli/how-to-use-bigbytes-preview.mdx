...
and also run [validation](/guides/cli/how-to-use-bigbytes-validate). This is only copied on preview creation, you can't sync the content afterwards.

### Run `bigbytes preview` from inside your project
...
When you create developer previews, Bigbytes will use the same warehouse connection settings you have in your
`profiles.yml` file for your current dbt project. This can be a problem if you're using a local database that your
laptop can reach but your Bigbytes instance cannot.

## Set up developer previews on your pull requests

If you've connected Bigbytes to GitHub, you can setup a `github action` and get Bigbytes to create new dynamic `preview` projects automatically
...
Now copy [this start-preview.yml file](https://github.com/bigbytes/cli-actions/blob/main/start-preview.yml) from our [cli-actions](https://github.com/bigbytes/cli-actions) repo

And save by clicking on `Start commit`

Do the same with [this close-preview.yml file](https://github.com/bigbytes/cli-actions/blob/main/close-preview.yml).

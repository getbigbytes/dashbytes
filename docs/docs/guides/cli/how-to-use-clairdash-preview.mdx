...
and also run [validation](/guides/cli/how-to-use-clairdash-validate). This is only copied on preview creation, you can't sync the content afterwards.

### Run `clairdash preview` from inside your project
...
Then `cmd` + `click` to open the preview link from your terminal. Once you're in Clairdash go to `Explore` --> `Tables`, then click on the model(s) you just updated to see your changes and play around with them.

**Problems with credentials?**

When you create developer previews, Clairdash will use the same warehouse connection settings you have in your
`profiles.yml` file for your current dbt project. This can be a problem if you're using a local database that your
laptop can reach but your Clairdash instance cannot.

## Set up developer previews on your pull requests

If you've connected Clairdash to GitHub, you can setup a `github action` and get Clairdash to create new dynamic `preview` projects automatically
...
If you haven't already set up a GitHub action for Clairdash, you'll need to add some secrets to GitHub. If you already have a GitHub action for Clairdash, then you can use the same Clairdash secrets you created for your other action.
...
Now copy [this start-preview.yml file](https://github.com/clairdash/cli-actions/blob/main/start-preview.yml) from our [cli-actions](https://github.com/clairdash/cli-actions) repo

And save by clicking on `Start commit`

Do the same with [this close-preview.yml file](https://github.com/clairdash/cli-actions/blob/main/close-preview.yml).
...
Then, update your GitHub action to use the username as the `--target` flag for the `clairdash start-preview` command.

```yaml
run: clairdash start-preview --project-dir "$PROJECT_DIR" --profiles-dir . --name ${GITHUB_REF##*/} --target ${{ github.actor }}
```
...
After that we need to add a new env variable to the step "Clairdash CLI start preview" which is the schema that dbt cloud will use.

...
You can [see the full list of metric types](/references/metrics.mdx#metric-types) that you can use in your Clairdash project.

We support metrics defined using either Clairdash or dbt syntax! You can read more about the two methods [in our reference docs here](/references/metrics.mdx#adding-metrics-to-your-project).

Once you've added your metrics, you can use them in Clairdash to build charts and filter results. Metrics appear in the Explore view, above dimensions and, if selected, pop us as brownish-yellow fields in your results table.
...
We're going to try adding a new metric to our dbt project, then syncing it with Clairdash. We've made this really easy to do using our CLI tool.
...
Once you've added a metric to your dbt model, you might want to check to make sure that it's working the way you'd expect. This is where `clairdash preview` comes in handy.
...
Then `cmd` + `click` to open the preview link from your terminal. Once you're in Clairdash go to `Explore` --> `Tables`, then click on the model you just updated to see your new metric and play around with it.
...
Things like the format, the label that people see in Clairdash, rounding, etc. - these are all configurations that you can apply to your metrics.
...
If you're working with a version controlled project, you'll just want to make sure to merge your changes into the branch you've connected to your Clairdash project (e.g. `main` or `master`).
...
And voilà! Your new metric is available to explore in Clairdash.

---

## Adding custom metrics in the Explore view

The fields that you see in your `dimensions` and `metrics` are created by the people maintaining your Clairdash project.

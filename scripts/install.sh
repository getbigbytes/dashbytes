...
CLAIRDASH_INSTALL_TYPE='bash_install'
dbt_project_dir=""
port=8080
...
track() {
  ...
  URL="https://analytics.clairdash.com/v1/track"
...
track_support() {
  echo -e "\n📨 🙏 Sorry that you had an issue with the installation. Please head to our Slack community and post your issue in #help 🙋 \n https://join.slack.com/t/clairdash-community/shared_invite/zt-2ehqnrvqt-LbCq7cUSFHAzEj_wMuxg4A \n Someone from Clairdash will help you out (usually the same day)"
}

# This function checks if the relevant ports required by Clairdash are available or not
...
check_ports_occupied() {
    ...
    if [[ -n $port_check_output ]]; then
        ...
        echo "+++++++++++ ERROR ++++++++++++++++++++++"
        echo "Clairdash requires ports $port to be open. Please shut down any other service(s) that may be running on these ports."
        echo "++++++++++++++++++++++++++++++++++++++++"
        echo ""
        exit 1
    fi
...
set_vars() {
    echo -e "⚙️  Enter your Clairdash settings:"
...
bye() {  # Prints a friendly good bye message and exits the script.
    if [ "$?" -ne 0 ]; then
        ...
        echo "Please reach us on Clairdash for support https://join.slack.com/t/clairdash-community/shared_invite/zt-2ehqnrvqt-LbCq7cUSFHAzEj_wMuxg4A"
        echo "++++++++++++++++++++++++++++++++++++++++"
...
echo -e "👋 Thank you for trying out Clairdash! "
echo ""
...
echo -e "👉 ${RED}Please enter how you want to setup Clairdash\n"
...
echo ""
echo -e "\n🟡 Pulling the latest container images for Clairdash.\n"
CLAIRDASH_INSTALL_ID="$INSTALLATION_ID" CLAIRDASH_INSTALL_TYPE="$CLAIRDASH_INSTALL_TYPE" docker compose --env-file ./.env.fast-install -f docker-compose.yml pull

echo ""
echo "🟡 Starting the Clairdash containers. It may take a few minutes ..."
echo
...
if [[ $setup_type == 'local_dbt' ]]; then
    CLAIRDASH_INSTALL_ID="$INSTALLATION_ID" CLAIRDASH_INSTALL_TYPE="$CLAIRDASH_INSTALL_TYPE" PORT="$port" DBT_PROJECT_DIR="$dbt_project_dir" docker compose --env-file ./.env.fast-install -f docker-compose.yml up --detach --remove-orphans || true
else
    CLAIRDASH_INSTALL_ID="$INSTALLATION_ID" CLAIRDASH_INSTALL_TYPE="$CLAIRDASH_INSTALL_TYPE" docker compose --env-file ./.env.fast-install -f docker-compose.yml up --detach --remove-orphans || true
fi
...
if [[ $status_code -ne 200 ]]; then
    ...
    echo "ℹ️  To restart Clairdash: docker compose -f docker-compose.yml start"
    echo "ℹ️  To stop Clairdash: docker compose -f docker-compose.yml stop -v"
    echo "ℹ️  To bring down Clairdash and clean volumes: docker compose -f docker-compose.yml down -v"

    echo ""
    echo "+++++++++++++++++++++++++++++++++++++++++++++++++"
    echo ""
    echo "👉 Need help Getting Started?"
    echo -e "Join us on Slack https://join.slack.com/t/clairdash-community/shared_invite/zt-2ehqnrvqt-LbCq7cUSFHAzEj_wMuxg4A"
    echo ""
